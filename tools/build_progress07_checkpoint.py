#!/usr/bin/env python3
"""Build the Progress 07 checkpoint from a clean committed source worktree and source-bound evidence."""
...
NEXT_CLUSTER = (
    "Submit the independently verified Progress 07 inner checkpoint and consolidated outer envelope for True North "
    "milestone-closure review. Progress 08 and production promotion remain unauthorized until separately approved."
)

AUTHORIZED_EPICS = {"OPS-001"}

def _scope_template() -> dict[str, Any]:
    path = ROOT / "requirements/MILESTONE_SCOPE_PROGRESS_07.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise RuntimeError("Progress 07 milestone scope must be an object")
    return value
...
def _scope_ids() -> frozenset[str]:
    ...
    if len(identifiers) != 64:
        raise RuntimeError(f"Progress 07 scope must contain 64 unique requirements, found {len(identifiers)}")
    if set(template.get("authorized_epics", [])) != AUTHORIZED_EPICS:
        raise RuntimeError("Progress 07 scope epics differ from the True North authorization")
    return identifiers
...
def _milestone_scope(*, ledger: dict[str, Any], facts: dict[str, Any]) -> dict[str, Any]:
    template = _scope_template()
    selected = {str(item.get("requirement_id")): item for item in ledger["requirements"] if str(item.get("requirement_id")) in P07_SCOPE_IDS}
    if set(selected) != set(P07_SCOPE_IDS):
        raise RuntimeError("Progress 07 source ledger does not contain the complete authorized scope")
    ...
    if included_ids | deferred_ids != set(P07_SCOPE_IDS) or included_ids & deferred_ids:
        raise RuntimeError("Progress 07 milestone scope is not an exact disjoint partition")
    ...
    return {
        ...
        "milestone": "Progress 07 bounded OPS-001 security and privacy readiness",
        "scope_statement": template["scope_statement"],
        "authoritative_wording": MILESTONE_WORDING,
        "scope_requirement_ids": sorted(P07_SCOPE_IDS),
        "included_requirements": included,
        "deferred_requirements": deferred,
        "summary": {
            "scope_requirements_total": len(P07_SCOPE_IDS),
...
def _render_status(facts: dict[str, Any], counts: dict[str, int]) -> str:
    lines = [
        "# SIP v1.1.0 Implementation Status — Progress 07",
        "",
        MILESTONE_WORDING,
        "",
    ...
    lines += [
        ...
        "## Release posture",
        "",
        "Progress 07 is a source-bound bounded security/privacy readiness checkpoint. Progress 08 is unauthorized and production deployment and release remain NO-GO.",
        "",
...
def _render_resume(facts: dict[str, Any]) -> str:
    return "\n".join([
        "# Resume Implementation — SIP v1.1.0 Progress 07",
        "",
        "This continuation record is generated from `build/checkpoints/progress-07.json`.",
        "",
        f"- Branch: `{facts['branch']}`",
...
def _render_coverage(facts: dict[str, Any], ledger: dict[str, Any]) -> str:
    ...
    lines = [
        "# Requirements coverage report — Progress 07",
        "",
        f"Checkpoint: `{facts['checkpoint_id']}`",
        f"Commit: `{facts['commit']}`",
        f"Source root: `{facts['source_tree_root_sha256']}`",
...
def _require_acceptance(report: dict[str, Any], *, commit: str, source_root: str) -> None:
    ...
    if report.get("checkpoint_id") != CHECKPOINT_ID or source.get("commit") != commit or source.get("source_tree_root_sha256") != source_root:
        raise RuntimeError("checkpoint acceptance report is not bound to Progress 07 source")
...
def build(*, evidence_root: Path, output: Path, branch: str, attestation_path: Path) -> dict[str, Any]:
    ...
    if python_tests < 300 or any(int(totals.get(key, 0)) for key in ("failures", "errors", "skipped")):
        raise RuntimeError("Python matrix does not satisfy the Progress 07 baseline")
    ...
    readiness = _load_json(evidence_root / "build/reports/release-readiness-progress-07.json")
    ...
    with tempfile.TemporaryDirectory(prefix="sip-progress-07-build-") as temporary:
        ...
        bundle = artifacts_dir / f"Spatial-Intelligence-Platform-v1.1.0-progress-07-{commit}.bundle"
        ...
        _write(stage / "build/checkpoints/progress-07.json", checkpoint_record)
        milestone = _milestone_scope(ledger=ledger, facts=facts)
        _write(stage / "MILESTONE_SCOPE_PROGRESS_07.json", milestone)
        ...
        latest = {
            ...
            "source_tree_root_sha256": source_root,
            "readiness": "blocked",
            "checkpoint_record": "build/checkpoints/progress-07.json",
            "progress_07_authorized": True,
            "progress_08_authorized": False,
            "production_authorized": False,
        }
        ...
        raw_paths: dict[str, list[str]] = {
            ...
            "demonstrations": [
                ...
                "build/evidence/demos/construction.json", "build/evidence/demos/liveforever.json", "build/evidence/demo-progress07-security.json",
                ...
                "build/evidence/gates/demo-construction.log", "build/evidence/gates/demo-liveforever.log", "build/evidence/gates/demo-security.log",
            ],
            ...
            "release_readiness": ["build/reports/release-readiness-progress-07.json", "build/reports/checkpoint-acceptance-gates.json", "build/reports/release-report.json", "build/evidence/gates/web-acceptance.log", "build/evidence/gates/release.log", "build/evidence/gates/release-mode.log"],
        }
        statuses = {
        ...
        report = "\n".join([
            "# Progress 07 Final Implementation Report",
            "",
            ...
            MILESTONE_WORDING,
            "",
            "## Bounded OPS-001 implementation",
            "",
            "Threat, privileged-access, workload-identity, key, privacy, audit, provider, runtime-safety, and supply-chain controls are implemented as the bounded Progress 07 OPS-001 milestone.",
            "",
            ...
            "",
            "All Progress 07 demonstrations use synthetic or non-sensitive fixtures. No confidential facility, critical-infrastructure, private-family, cloned voice, simulated likeness, or unapproved external-provider data is included.",
            "",
            ...
            "",
            "Append-only schema revision 0017_progress07_security_privacy_readiness is included. All earlier migrations are preserved unchanged.",
            "",
            "Progress 07 is authorized only as this bounded development milestone. Progress 08 and production deployment/release remain unauthorized and NO-GO.",
        ]) + "\n"
        (stage / "FINAL_IMPLEMENTATION_REPORT.md").write_text(report, encoding="utf-8")
        milestone_report = "\n".join([
            "# Progress 07 Milestone Report",
            "",
            f"Checkpoint: `{CHECKPOINT_ID}`",
            f"Commit: `{commit}`",
            f"Source root: `{source_root}`",
            ...
            MILESTONE_WORDING,
            "",
            f"- Bounded OPS-001 scope: {len(P07_SCOPE_IDS)} requirements, with every included/deferred item explicitly partitioned.",
            ...
            f"Python matrix: {python_tests} passed, 0 failed, 0 errors, 0 skipped.",
            "",
            "Progress 08 remains unauthorized pending independent True North review of Progress 07.",
        ]) + "\n"
        (stage / "PROGRESS_07_MILESTONE_REPORT.md").write_text(milestone_report, encoding="utf-8")
        checkpoint_readme = "\n".join([
            "# SIP v1.1.0 Progress 07 checkpoint",
            "",
            ...
            "",
            "```bash",
            "python source/tools/verify_progress07_checkpoint.py Spatial-Intelligence-Platform-v1.1.0-progress-07.zip",
            "```",
            "",
            "Clone the included Git bundle:",
            "",
            "```bash",
            f"git clone -b {branch} artifacts/Spatial-Intelligence-Platform-v1.1.0-progress-07-{commit}.bundle <destination>",
            "```",
            "",
...
def main() -> None:
    ...
    parser.add_argument("--branch", default="progress-07-bounded")
