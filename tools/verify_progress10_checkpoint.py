#!/usr/bin/env python3
"""Independently verify a SIP Progress 10 checkpoint ZIP and all retained evidence."""
...
EXPECTED_BASE_COMMIT = "f293d6425182ef04f5893275724e2a4bd28cf00e"
EXPECTED_BASE_ZIP_SHA256 = "edf8aa0eabf5874b97b5090cb43e6d4867fff97291d637e57b44fd109fa4cf6e"
EXPECTED_BASE_OUTER_SHA256 = "2caf854b4457c1a3da451a6f96f6652e82da71e37224c452e20cbdf69e92e2be"
EXPECTED_BASE_SOURCE_ROOT = "7043fbee94838d69fb5b70afc570f9004acc21b9110907f6a551fe3c1db3cdcb"
EXPECTED_MIGRATION_SHA256 = "4a306e130ff3785128d3f281a995859daf390f39c971713b888a5c1a32084dbb"
EXPECTED_MIGRATION_BYTES = 29254
EXPECTED_SCOPE_TOTAL = 45
...
def _verify_accepted_base_ancestry(
    ...
    """Prove the checkpoint commit descends from the accepted Progress 09 base."""
    ...
    if not bundle.is_file():
        return
    with tempfile.TemporaryDirectory(prefix="sip-progress10-ancestry-") as temporary:
        ...
        if base_exists.returncode != 0:
            verification.fail("BASE_COMMIT_MISSING", "accepted Progress 09 commit is absent from the Git bundle", bundle_relative)
            return
        ...
        if ancestry.returncode != 0:
            verification.fail(
                ...
                "checkpoint commit does not descend from accepted Progress 09",
                bundle_relative,
            )
...
def _verify_predecessor(root: Path, verification: Verification) -> None:
    ...
    accepted = package_record.get("accepted_progress_09_checkpoint")
    if not isinstance(accepted, dict):
        verification.fail("PREDECESSOR_PROGRESS09_MISSING", "accepted Progress 09 record is absent", relative)
        return
    for key, expected in (
        ...
        if accepted.get(key) != expected:
            verification.fail("PREDECESSOR_PROGRESS09_MISMATCH", f"{key} differs from the accepted Progress 09 base", relative)
...
def _verify_scope_and_traceability(root: Path, verification: Verification, facts: dict[str, Any]) -> None:
    package_relative = "MILESTONE_SCOPE_PROGRESS_10.json"
    package_scope = _read_json(root / package_relative, verification, code="MILESTONE_SCOPE_INVALID")
    source_scope = _read_json(root / "source/requirements/MILESTONE_SCOPE_PROGRESS_10.json", verification, code="SOURCE_MILESTONE_SCOPE_INVALID")
    ...
    audit = _read_json(root / "source/requirements/progress-10-traceability-audit.json", verification, code="TRACEABILITY_AUDIT_INVALID")
    ...
    if package_scope.get("authoritative_wording") != MILESTONE_WORDING:
        verification.fail("MILESTONE_WORDING", "authoritative wording differs from the authorized Progress 10 scope", package_relative)
    ...
    if package_scope.get("progress_10_authorized") is not True or package_scope.get("progress_11_authorized") is not False or package_scope.get("production_authorized") is not False:
        verification.fail("MILESTONE_POSTURE", "Progress 10, Progress 11, or production posture is incorrect", package_relative)
    ...
    if set(included) & set(deferred) or len(set(included) | set(deferred)) != EXPECTED_SCOPE_TOTAL:
        verification.fail("MILESTONE_PARTITION", "Progress 10 milestone scope is not an exact 45-requirement partition", package_relative)
    ...
    if audit.get("status") != "passed_complete" or int(audit.get("finding_count", -1)) != 0:
        verification.fail("TRACEABILITY_AUDIT", "Progress 10 traceability audit did not pass with zero findings", "source/requirements/progress-10-traceability-audit.json")
    if int(audit.get("requirement_count", -1)) != EXPECTED_SCOPE_TOTAL:
        verification.fail("TRACEABILITY_SCOPE", "Progress 10 traceability audit did not cover all 45 scoped requirements", "source/requirements/progress-10-traceability-audit.json")
    if audit.get("progress_11_authorized") is not False or audit.get("production_authorized") is not False:
        verification.fail("TRACEABILITY_POSTURE", "traceability audit incorrectly authorizes Progress 11 or production", "source/requirements/progress-10-traceability-audit.json")
    ...
    if len(audited) != EXPECTED_SCOPE_TOTAL:
        verification.fail("TRACEABILITY_RECORD_COUNT", "traceability audit record count differs", "source/requirements/progress-10-traceability-audit.json")
    if {str(item.get("requirement_id")) for item in audited if isinstance(item, dict)} != (set(included) | set(deferred)):
        verification.fail("TRACEABILITY_SCOPE_DRIFT", "traceability audit requirement set differs from the Progress 10 milestone scope", "source/requirements/progress-10-traceability-audit.json")
    for record in audited:
        if not isinstance(record, dict):
            verification.fail("TRACEABILITY_RECORD", "audit record must be an object", "source/requirements/progress-10-traceability-audit.json")
            continue
        ...
        if requirement_id in included and not direct:
            verification.fail("TRACEABILITY_DIRECT_EVIDENCE", f"{record.get('requirement_id')} has no direct Progress 10 evidence", "source/requirements/progress-10-traceability-audit.json")
        for test in direct:
            if not isinstance(test, dict) or test.get("exists") is not True or test.get("declares_requirement_id") is not True or test.get("present_in_implementation_map") is not True:
                verification.fail("TRACEABILITY_DECLARATION", f"{record.get('requirement_id')} has invalid direct test evidence", "source/requirements/progress-10-traceability-audit.json")
...
def _verify_acceptance(root: Path, verification: Verification, facts: dict[str, Any]) -> None:
    ...
    if report.get("progress_10_authorized") is not True or report.get("progress_11_authorized") is not False or report.get("production_authorized") is not False:
    ...
    readiness_relative = "build/reports/release-readiness-progress-10.json"
    ...
    if readiness:
        ...
        if readiness.get("status") != "blocked" or readiness.get("production_authorized") is not False or readiness.get("progress_10_authorized") is not True or readiness.get("progress_11_authorized") is not False:
        ...
        if "True North" not in next_action or "Progress 10" not in next_action:
...
def _verify_migration(root: Path, verification: Verification) -> None:
    relative = "source/migrations/versions/0020_progress10_recovery_retention.py"
    migration = root / relative
    if not migration.is_file():
        verification.fail("MIGRATION_0020_MISSING", "append-only Progress 10 recovery migration is missing", relative)
        return
    if migration.stat().st_size != EXPECTED_MIGRATION_BYTES or sha256_file(migration) != EXPECTED_MIGRATION_SHA256:
        verification.fail("MIGRATION_0020_HASH", "Progress 10 migration bytes differ from the locked revision", relative)
    ...
    matched = [item for item in records if isinstance(item, dict) and item.get("path") == "migrations/versions/0020_progress10_recovery_retention.py"]
    if len(matched) != 1:
        verification.fail("MIGRATION_0020_MANIFEST", "migration manifest does not contain exactly one 0020 record", "source/migrations/manifest.json")
    else:
        item = matched[0]
        if int(item.get("byte_count", -1)) != EXPECTED_MIGRATION_BYTES or item.get("sha256") != EXPECTED_MIGRATION_SHA256:
            verification.fail("MIGRATION_0020_MANIFEST_HASH", "migration manifest lock differs", "source/migrations/manifest.json")


def _verify_status_and_evidence(root: Path, verification: Verification, source_record: dict[str, Any] | None) -> None:
    checkpoint_relative = "build/checkpoints/progress-10.json"
    ...
    required = {
        ...
        "progress_10_authorized", "progress_11_authorized", "production_authorized",
    }
    for field in sorted(required - set(facts)):
        verification.fail("CHECKPOINT_FACT_MISSING", f"missing fact {field}", checkpoint_relative)
    if facts.get("checkpoint_id") != CHECKPOINT_ID or facts.get("branch") != "progress-10-recovery":
    ...
    if int(facts.get("python_tests_passed", -1)) < MINIMUM_PYTHON_TESTS:
        verification.fail("CHECKPOINT_TEST_BASELINE", "Python test count is below the Progress 10 minimum", checkpoint_relative)
    ...
    if facts.get("progress_10_authorized") is not True or facts.get("progress_11_authorized") is not False or facts.get("production_authorized") is not False:
        verification.fail("CHECKPOINT_POSTURE", "Progress 10, Progress 11, or production posture is incorrect", checkpoint_relative)
    if checkpoint.get("milestone_wording") != MILESTONE_WORDING:
        verification.fail("CHECKPOINT_WORDING", "checkpoint wording differs from the authorized Progress 10 scope", checkpoint_relative)
    if source_record:
    ...
    if latest:
        ...
        if latest.get("progress_10_authorized") is not True or latest.get("progress_11_authorized") is not False or latest.get("production_authorized") is not False:
            verification.fail("LATEST_POSTURE", "latest record has incorrect Progress 10/10 or production posture", "build/release/latest.json")
    ...
    required_docs = (
        ...
        "FINAL_IMPLEMENTATION_REPORT.md", "PROGRESS_10_MILESTONE_REPORT.md",
    )
    for relative in required_docs:
        ...
        if "Progress 10" not in text or "Progress 11" not in text or not any(token in text for token in ("NO-GO", "blocked", "unauthorized", "not authorized")):
...
def verify_archive(archive_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="sip-progress10-verify-") as temporary:
