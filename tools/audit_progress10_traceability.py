#!/usr/bin/env python3
"""Audit the exact bounded Progress 10 OPS-004 milestone against executable evidence."""
...
def build() -> dict[str, Any]:
    ...
    if scope.get("accepted_base_commit") != EXPECTED_BASE:
        findings.append({"code": "ACCEPTED_BASE_MISMATCH"})
    if scope.get("authorized_epics") != ["OPS-004"]:
        findings.append({"code": "AUTHORIZED_EPIC_MISMATCH"})
    ...
    if scope.get("progress_10_authorized") is not True or scope.get("progress_11_authorized") is not False or scope.get("production_authorized") is not False:
    ...
    invalid = sorted(item for item in included_ids if not item.startswith(ALLOWED_PREFIXES))
    if invalid:
        findings.append({"code": "OUT_OF_SCOPE_REQUIREMENT", "requirements": invalid})
    for entry in sorted(included, key=lambda value: value["requirement_id"]):
        ...
        for raw in entry.get("direct_tests", []):
            ...
            direct.append({"test_id": test_id, "exists": exists, "declares_requirement_id": bool(declares), "present_in_implementation_map": bool(mapped), "declared_requirement_ids": sorted(declared or [])})
            if not exists:
        ...
        if entry.get("implementation_status") != ledger_item.get("implementation_status"):
            findings.append({"code": "SCOPE_LEDGER_STATUS_MISMATCH", "requirement_id": rid, "scope": entry.get("implementation_status"), "ledger": ledger_item.get("implementation_status")})
        ...
        records.append({"requirement_id": rid, "implementation_status": ledger_item.get("implementation_status"), "direct_tests": direct, "implementation_files": entry.get("implementation_files", []), "evidence_paths": entry.get("evidence_paths", [])})
    ...
    return {
        "schema": "sip.progress10-traceability-audit/v1",
        "milestone": "Progress 10 bounded backup, restore, disaster recovery, retention, and deletion",
...
def main() -> None:
    ...
    if args.check:
        if not AUDIT.is_file() or AUDIT.read_text(encoding="utf-8") != text:
            raise SystemExit("Progress 10 traceability audit drift detected; run tools/audit_progress10_traceability.py")
    else:
