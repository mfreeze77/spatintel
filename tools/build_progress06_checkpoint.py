#!/usr/bin/env python3
"""Build the Progress 06 checkpoint from a clean committed source worktree and source-bound evidence."""
...
NEXT_CLUSTER = (
    "Submit the independently verified Progress 06 inner checkpoint and consolidated outer envelope for True North "
    "review. Progress 07 and production promotion remain unauthorized until separately approved."
)
...
def _scope_template() -> dict[str, Any]:
    path = ROOT / "requirements/MILESTONE_SCOPE_PROGRESS_06.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise RuntimeError("Progress 06 milestone scope must be an object")
    return value
...
def _scope_ids() -> frozenset[str]:
    ...
    if len(identifiers) != 206:
        raise RuntimeError(f"Progress 06 scope must contain 206 unique requirements, found {len(identifiers)}")
    if set(template.get("authorized_epics", [])) != AUTHORIZED_EPICS:
        raise RuntimeError("Progress 06 scope epics differ from the True North authorization")
    return identifiers
...
def _milestone_scope(*, ledger: dict[str, Any], facts: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(selected) != set(P06_SCOPE_IDS):
        raise RuntimeError("Progress 06 source ledger does not contain the complete authorized scope")
    ...
    if included_ids | deferred_ids != set(P06_SCOPE_IDS) or included_ids & deferred_ids:
        raise RuntimeError("Progress 06 milestone scope is not an exact disjoint partition")
...
def _render_status(facts: dict[str, Any], counts: dict[str, int]) -> str:
    lines = [
        "# SIP v1.1.0 Implementation Status — Progress 06",
        "",
        MILESTONE_WORDING,
        "",
    ...
    lines += [
        ...
        "## Release posture",
        "",
        "Progress 06 is a source-bound vertical MVP checkpoint. Progress 07 is unauthorized and production deployment and release remain NO-GO.",
        "",
...
def _render_resume(facts: dict[str, Any]) -> str:
    return "\n".join([
        "# Resume Implementation — SIP v1.1.0 Progress 06",
        "",
        "This continuation record is generated from `build/checkpoints/progress-06.json`.",
        "",
        f"- Branch: `{facts['branch']}`",
...
def _render_coverage(facts: dict[str, Any], ledger: dict[str, Any]) -> str:
    ...
    lines = [
        "# Requirements coverage report — Progress 06",
        "",
        f"Checkpoint: `{facts['checkpoint_id']}`",
        f"Commit: `{facts['commit']}`",
        f"Source root: `{facts['source_tree_root_sha256']}`",
...
def _require_acceptance(report: dict[str, Any], *, commit: str, source_root: str) -> None:
    ...
    if report.get("checkpoint_id") != CHECKPOINT_ID or source.get("commit") != commit or source.get("source_tree_root_sha256") != source_root:
        raise RuntimeError("checkpoint acceptance report is not bound to Progress 06 source")
...
def build(*, evidence_root: Path, output: Path, branch: str, attestation_path: Path) -> dict[str, Any]:
    ...
    if python_tests < 265 or any(int(totals.get(key, 0)) for key in ("failures", "errors", "skipped")):
        raise RuntimeError("Python matrix does not satisfy the Progress 06 baseline")
    ...
    readiness = _load_json(evidence_root / "build/reports/release-readiness-progress-06.json")
    ...
    with tempfile.TemporaryDirectory(prefix="sip-progress-06-build-") as temporary:
        ...
        bundle = artifacts_dir / f"Spatial-Intelligence-Platform-v1.1.0-progress-06-{commit}.bundle"
        ...
        _write(stage / "build/checkpoints/progress-06.json", checkpoint_record)
        milestone = _milestone_scope(ledger=ledger, facts=facts)
        _write(stage / "MILESTONE_SCOPE_PROGRESS_06.json", milestone)
        ...
        latest = {
            ...
            "source_tree_root_sha256": source_root,
            "readiness": "blocked",
            "checkpoint_record": "build/checkpoints/progress-06.json",
            "progress_07_authorized": False,
            "production_authorized": False,
        }
        ...
        raw_paths: dict[str, list[str]] = {
            ...
            "migrations": ["build/reports/tests/migration.xml", "build/reports/tests/migration-direct.xml", "build/evidence/gates/migrations.log", "source/migrations/versions/0012_scene_runtime_review.py", "source/migrations/versions/0013_scene_change_application_atomicity.py", "source/migrations/versions/0014_vertical_mvp.py"],
            ...
            "requirements": ["source/requirements/requirements-ledger.json", "source/requirements/requirements-ledger.csv", "source/requirements/requirements-ledger.sqlite", "source/requirements/implementation-map.json", "source/requirements/MILESTONE_SCOPE_PROGRESS_06.json", "source/requirements/progress-06-traceability-audit.json", "build/reports/spec-lint.json", "build/evidence/gates/traceability-evidence.log"],
            ...
            "release_readiness": ["build/reports/release-readiness-progress-06.json", "build/reports/checkpoint-acceptance-gates.json", "build/reports/release-report.json", "build/evidence/gates/web-acceptance.log", "build/evidence/gates/release.log", "build/evidence/gates/release-mode.log"],
        }
        statuses = {
        ...
        report = "\n".join([
            "# Progress 06 Final Implementation Report",
            "",
            ...
            MILESTONE_WORDING,
            "",
            "## Implemented vertical MVPs",
            "",
            "Construction includes governed project/building/level/zone/room survey planning and field visits; fire-alarm, access-control, BAS, mechanical, and electrical entity packs; immutable drawing and RFI revisions; issue, correction, retest, commissioning, IFC/BCF, deterministic reporting, and open owner-handoff workflows.",
            "",
            "LiveForever includes people, relationships, places, objects, events, memories, interviews, timestamped transcript segments, conflicting recollections, immutable family corrections, granular consent and governance, generated-presence gates, quiet/evidence/safe-exit modes, narrative editions, revocation propagation, and open offline preservation.",
            "",
            "All Progress 06 demonstrations use synthetic or non-sensitive fixtures. No confidential facility, critical-infrastructure, private-family, cloned voice, simulated likeness, or unapproved external-provider data is included.",
            "",
            ...
            "",
            "Append-only schema revision 0014_vertical_mvp is included. Migrations 0012 and 0013 are preserved unchanged.",
            "",
        ...
        milestone_report = "\n".join([
            "# Progress 06 Vertical MVP Milestone Report",
            "",
            f"Checkpoint: `{CHECKPOINT_ID}`",
            f"Commit: `{commit}`",
            f"Source root: `{source_root}`",
            ...
            MILESTONE_WORDING,
            "",
            f"- Authorized requirement scope: {len(P06_SCOPE_IDS)} requirements across CON-001 through CON-005 and LIF-001 through LIF-004.",
            ...
            f"Python matrix: {python_tests} passed, 0 failed, 0 errors, 0 skipped.",
            "",
            "Progress 07 remains unauthorized pending independent True North review of Progress 06.",
        ]) + "\n"
        (stage / "PROGRESS_06_MILESTONE_REPORT.md").write_text(milestone_report, encoding="utf-8")
        checkpoint_readme = "\n".join([
            "# SIP v1.1.0 Progress 06 checkpoint",
            "",
            ...
            "",
            "```bash",
            "python source/tools/verify_progress06_checkpoint.py Spatial-Intelligence-Platform-v1.1.0-progress-06.zip",
            "```",
            "",
            "Clone the included Git bundle:",
            "",
            "```bash",
            f"git clone -b {branch} artifacts/Spatial-Intelligence-Platform-v1.1.0-progress-06-{commit}.bundle <destination>",
            "```",
            "",
...
def main() -> None:
    ...
    parser.add_argument("--branch", default="progress-06-vertical-mvps")
