#!/usr/bin/env python3
"""Build the Progress 06-R1 checkpoint from a clean committed source worktree and source-bound evidence."""
...
CHECKPOINT_ID = "sip-v1.1.0-progress-06-r1"
TOP_LEVEL = "Spatial-Intelligence-Platform-v1.1.0-progress-06-r1"
EXPECTED_SPEC_SHA256 = "84b570464b98b1baf3107789af22edad2a14086e5420df35a961f5fb16980bb8"
MILESTONE_WORDING = (
    "Progress 06-R1 is a narrow remediation checkpoint for the accepted Progress 06 development snapshot. "
    "It closes the scoped cross-tenant mutation, authenticated-verifier, restricted-export approval, portable-package "
    "integrity, immutable-provenance, replay-validation, and hermetic-test-runner stop-lines. Progress 07 is not "
    "authorized, PLTVIEW-007 remains IMPLEMENTED_UNVERIFIED, and production promotion remains NO-GO."
)
...
EXTERNAL_GAPS = [
    ...
    "Docker Compose runtime, Kubernetes deployment, Terraform provider execution, and credentialed cloud recovery were not available; structural checks are not deployment validation.",
...
NEXT_CLUSTER = (
    "Submit the independently verified Progress 06-R1 inner checkpoint and consolidated outer envelope for True North "
    "milestone-closure review. Progress 07 and production promotion remain unauthorized until separately approved."
)
...
def _scope_template() -> dict[str, Any]:
    path = ROOT / "requirements/MILESTONE_SCOPE_PROGRESS_06_R1.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise RuntimeError("Progress 06-R1 milestone scope must be an object")
    return value
...
def _scope_ids() -> frozenset[str]:
    ...
    if len(identifiers) != 18:
        raise RuntimeError(f"Progress 06-R1 scope must contain 18 unique remediation requirements, found {len(identifiers)}")
    if set(template.get("authorized_epics", [])) != AUTHORIZED_EPICS:
        raise RuntimeError("Progress 06-R1 scope epics differ from the True North authorization")
    return identifiers

P06_SCOPE_IDS = _scope_ids()
...
def _milestone_scope(*, ledger: dict[str, Any], facts: dict[str, Any]) -> dict[str, Any]:
    template = _scope_template()
    selected = {str(item.get("requirement_id")): item for item in ledger["requirements"] if str(item.get("requirement_id")) in P06_SCOPE_IDS}
    if set(selected) != set(P06_SCOPE_IDS):
        raise RuntimeError("Progress 06-R1 source ledger does not contain the complete authorized scope")
    ...
    if included_ids | deferred_ids != set(P06_SCOPE_IDS) or included_ids & deferred_ids:
        raise RuntimeError("Progress 06-R1 milestone scope is not an exact disjoint partition")
    def rebound(requirement_id: str, *, deferred: bool) -> dict[str, Any]:
        ...
        if deferred and status != "NOT_STARTED":
            raise RuntimeError(f"deferred requirement {requirement_id} is not NOT_STARTED")
...
def _render_status(facts: dict[str, Any], counts: dict[str, int]) -> str:
    lines = [
        "# SIP v1.1.0 Implementation Status — Progress 06-R1",
        "",
        MILESTONE_WORDING,
        "",
    ...
    lines += [
        ...
        "## Release posture",
        "",
        "Progress 06-R1 is a source-bound narrow remediation checkpoint for the accepted Progress 06 snapshot. Progress 07 is unauthorized and production deployment and release remain NO-GO.",
        "",
...
def _render_resume(facts: dict[str, Any]) -> str:
    return "\n".join([
        "# Resume Implementation — SIP v1.1.0 Progress 06-R1",
        "",
        "This continuation record is generated from `build/checkpoints/progress-06-r1.json`.",
        "",
        f"- Branch: `{facts['branch']}`",
...
def _render_coverage(facts: dict[str, Any], ledger: dict[str, Any]) -> str:
    ...
    lines = [
        "# Requirements coverage report — Progress 06-R1",
        "",
        f"Checkpoint: `{facts['checkpoint_id']}`",
        f"Commit: `{facts['commit']}`",
        f"Source root: `{facts['source_tree_root_sha256']}`",
    ...
    lines += [
        ...
        "",
        "- Progress 07 remains **unauthorized** pending independent True North acceptance of Progress 06-R1.",
...
def _require_acceptance(report: dict[str, Any], *, commit: str, source_root: str) -> None:
    ...
    if report.get("checkpoint_id") != CHECKPOINT_ID or source.get("commit") != commit or source.get("source_tree_root_sha256") != source_root:
        raise RuntimeError("checkpoint acceptance report is not bound to Progress 06-R1 source")
...
def build(*, evidence_root: Path, output: Path, branch: str, attestation_path: Path) -> dict[str, Any]:
    ...
    if python_tests < 285 or any(int(totals.get(key, 0)) for key in ("failures", "errors", "skipped")):
        raise RuntimeError("Python matrix does not satisfy the Progress 06-R1 baseline")
    ...
    readiness = _load_json(evidence_root / "build/reports/release-readiness-progress-06-r1.json")
    ...
    with tempfile.TemporaryDirectory(prefix="sip-progress-06-r1-build-") as temporary:
        ...
        bundle = artifacts_dir / f"Spatial-Intelligence-Platform-v1.1.0-progress-06-r1-{commit}.bundle"
        ...
        _write(stage / "build/checkpoints/progress-06-r1.json", checkpoint_record)
        milestone = _milestone_scope(ledger=ledger, facts=facts)
        _write(stage / "MILESTONE_SCOPE_PROGRESS_06_R1.json", milestone)
        ...
        latest = {
            ...
            "source_tree_root_sha256": source_root,
            "readiness": "blocked",
            "checkpoint_record": "build/checkpoints/progress-06-r1.json",
            "progress_07_authorized": False,
            "production_authorized": False,
        }
        ...
        raw_paths: dict[str, list[str]] = {
            ...
            "migrations": ["build/reports/tests/migration.xml", "build/reports/tests/migration-direct.xml", "build/evidence/gates/migrations.log", "source/migrations/versions/0012_scene_runtime_review.py", "source/migrations/versions/0013_scene_change_application_atomicity.py", "source/migrations/versions/0014_vertical_mvp.py", "source/migrations/versions/0015_progress06_r1_security_controls.py"],
            ...
            "requirements": ["source/requirements/requirements-ledger.json", "source/requirements/requirements-ledger.csv", "source/requirements/requirements-ledger.sqlite", "source/requirements/implementation-map.json", "source/requirements/MILESTONE_SCOPE_PROGRESS_06.json", "source/requirements/progress-06-traceability-audit.json", "source/requirements/MILESTONE_SCOPE_PROGRESS_06_R1.json", "source/requirements/progress-06-r1-traceability-audit.json", "build/reports/spec-lint.json", "build/evidence/gates/traceability-evidence.log"],
            ...
            "release_readiness": ["build/reports/release-readiness-progress-06-r1.json", "build/reports/checkpoint-acceptance-gates.json", "build/reports/release-report.json", "build/evidence/gates/web-acceptance.log", "build/evidence/gates/release.log", "build/evidence/gates/release-mode.log"],
        }
        statuses = {
        ...
        report = "\n".join([
            "# Progress 06-R1 Final Implementation Report",
            "",
            ...
            MILESTONE_WORDING,
            "",
            "## Stop-line remediation",
            "",
            "Construction mutations are tenant/project scoped, issue verification derives from the authenticated principal, restricted annexes require immutable server-side approval, provenance assets are validated, and owner-handoff packages require exact complete checksum coverage and replay validation.",
            "",
            "LiveForever consent revocation is tenant/project scoped and preservation packages require exact complete checksum coverage and replay validation. Parallel Python shards use unique database and runtime roots.",
            "",
            "All Progress 06-R1 demonstrations use synthetic or non-sensitive fixtures. No confidential facility, critical-infrastructure, private-family, cloned voice, simulated likeness, or unapproved external-provider data is included.",
            "",
            ...
            "",
            "Append-only schema revision 0015_progress06_r1_security_controls is included. Migrations 0012, 0013, and 0014 are preserved unchanged.",
            "",
            "Progress 07 is not authorized. Production deployment and release remain NO-GO.",
        ]) + "\n"
        ...
        milestone_report = "\n".join([
            ...
            MILESTONE_WORDING,
            "",
            f"- Narrow remediation scope: {len(P06_SCOPE_IDS)} directly affected requirements; the accepted 206-requirement Progress 06 scope remains separately retained.",
            ...
            f"Python matrix: {python_tests} passed, 0 failed, 0 errors, 0 skipped.",
            "",
            "Progress 07 remains unauthorized pending independent True North review of Progress 06-R1.",
        ]) + "\n"
        (stage / "PROGRESS_06_R1_REMEDIATION_REPORT.md").write_text(milestone_report, encoding="utf-8")
        checkpoint_readme = "\n".join([
            "# SIP v1.1.0 Progress 06-R1 checkpoint",
            "",
            ...
            "",
            "```bash",
            "python source/tools/verify_progress06_r1_checkpoint.py Spatial-Intelligence-Platform-v1.1.0-progress-06-r1.zip",
            "```",
            "",
            "Clone the included Git bundle:",
            "",
            "```bash",
            f"git clone -b {branch} artifacts/Spatial-Intelligence-Platform-v1.1.0-progress-06-r1-{commit}.bundle <destination>",
            "```",
            "",
...
def main() -> None:
    ...
    parser.add_argument("--branch", default="progress-06-r1-remediation")
