#!/usr/bin/env python3
"""Audit the exact bounded Progress 11 QA-002 milestone against executable evidence."""
...
SCOPE = ROOT / "requirements/MILESTONE_SCOPE_PROGRESS_11.json"
AUDIT = ROOT / "requirements/progress-11-traceability-audit.json"
REQ = re.compile(r"\b[A-Z][A-Z0-9]+-\d{3}\b")
EXPECTED_BASE = "ab409f6ac7ca583535f69e5806b7a3bdbfe08214"
EXPECTED_INCLUDED = 96
EXPECTED_DEFERRED = 0
ALLOWED_PREFIXES = (
    "CONTEST-", "LIFTEST-", "TSTSTRAT-", "TSTSEC-", "TSTGATE-",
    "DELMVP-", "DELROAD-", "DELDOD-", "HYBTEST-", "APPFAIL-", "SIPMIG-",
)
...
def build() -> dict[str, Any]:
    ...
    if scope.get("authorized_epics") != ["QA-002"]:
    ...
    if scope.get("progress_11_authorized") is not True or scope.get("progress_12_authorized") is not False or scope.get("production_authorized") is not False:
        findings.append({"code": "AUTHORIZATION_POSTURE_INVALID"})

    included_ids = [item.get("requirement_id") for item in included]
    if len(set(included_ids)) != EXPECTED_INCLUDED:
        findings.append({"code": "DUPLICATE_OR_MISSING_SCOPE_IDS"})
    invalid_ids = sorted(identifier for identifier in included_ids if not isinstance(identifier, str) or not identifier.startswith(ALLOWED_PREFIXES))
    if invalid_ids:
    ...
    for entry in sorted(included, key=lambda value: value["requirement_id"]):
        ...
        if entry.get("implementation_status") != ledger_item.get("implementation_status"):
            findings.append({
                "code": "SCOPE_LEDGER_STATUS_MISMATCH", "requirement_id": rid,
                "scope": entry.get("implementation_status"), "ledger": ledger_item.get("implementation_status"),
            })
        if overlay is not None and entry.get("implementation_status") != overlay.get("implementation_status"):
            findings.append({
                "code": "SCOPE_MAP_STATUS_MISMATCH", "requirement_id": rid,
                "scope": entry.get("implementation_status"), "map": overlay.get("implementation_status"),
            })
    ...
    return {
        "schema": "sip.progress11-traceability-audit/v1",
        "milestone": "Progress 11 bounded QA-002 dual-vertical acceptance and release gates",
        ...
        "included_requirement_count": len(set(included_ids)),
...
def main() -> None:
    ...
    if args.check:
        if not AUDIT.is_file() or AUDIT.read_text(encoding="utf-8") != text:
            raise SystemExit("Progress 11 traceability audit drift detected; run tools/audit_progress11_traceability.py")
    else:
