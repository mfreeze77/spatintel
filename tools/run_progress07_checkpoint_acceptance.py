#!/usr/bin/env python3
"""Run the Progress 07 clean-source acceptance sequence and retain source-bound gate evidence."""
...
def _exclusive_acceptance_lock():
    ...
    with lock_path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            raise RuntimeError("Progress 07 checkpoint acceptance is already running") from exc
        handle.seek(0)
        handle.truncate()
...
def run(*, attestation_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "release_block_reason": "Progress 07 requires independent True North milestone review; Progress 08 and production remain unauthorized, and external validation gaps remain open.",
    }
...
def main() -> None:
    ...
    with _exclusive_acceptance_lock():
        ...
        "progress_08_authorized": False,
            ...
            "next_action": "Submit the independently verified Progress 07 inner checkpoint and consolidated outer envelope for True North review; Progress 08 and production remain unauthorized.",
        }
        readiness_path = ROOT / "build/reports/release-readiness-progress-07.json"
