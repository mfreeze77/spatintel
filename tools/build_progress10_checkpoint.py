#!/usr/bin/env python3
"""Build the Progress 10 checkpoint from a clean committed source worktree and source-bound evidence."""
...
TOP_LEVEL = "Spatial-Intelligence-Platform-v1.1.0-progress-10"
EXPECTED_SPEC_SHA256 = "84b570464b98b1baf3107789af22edad2a14086e5420df35a961f5fb16980bb8"
MILESTONE_WORDING = (
    "Progress 10 implements and verifies the bounded OPS-004 backup, restore, disaster-recovery, retention, deletion, preservation, portability-contingency, and conservative migration layer defined by the enclosed milestone scope. "
    "Executed managed PITR, object-version restoration, KMS recovery ceremonies, multi-region loss, physical edge recovery, customer-data exercises, and external witnessing remain external; local and synthetic recovery evidence is not production disaster-recovery certification. "
    "Progress 11 is unauthorized and production remains NO-GO."
)
...
EXTERNAL_GAPS = [
    ...
    "Managed database PITR, object-version restore, KMS recovery, queue/CDN continuity, multi-region loss, and credentialed cloud recovery were not executed; local restore and structural cloud definitions are not production DR validation.",
    ...
    "Approved LingBot-Map checkpoint bytes, commercial model-rights approval, CUDA/GPU execution, physical edge recovery, and real-scene validation remain external.",
...
NEXT_CLUSTER = (
    "Submit the independently verified Progress 10 inner checkpoint and consolidated outer envelope for True North "
    "milestone-closure review. Progress 11 and production promotion remain unauthorized until separately approved."
)

AUTHORIZED_EPICS = {"OPS-004"}

def _scope_template() -> dict[str, Any]:
    path = ROOT / "requirements/MILESTONE_SCOPE_PROGRESS_10.json"
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise RuntimeError("Progress 10 milestone scope must be an object")
    return value
...
def _scope_ids() -> frozenset[str]:
    ...
    if len(identifiers) != 45:
        raise RuntimeError(f"Progress 10 scope must contain 45 unique requirements, found {len(identifiers)}")
    if set(template.get("authorized_epics", [])) != AUTHORIZED_EPICS:
        raise RuntimeError("Progress 10 scope epics differ from the True North authorization")
    return identifiers

P10_SCOPE_IDS = _scope_ids()
...
def _milestone_scope(*, ledger: dict[str, Any], facts: dict[str, Any]) -> dict[str, Any]:
    template = _scope_template()
    selected = {str(item.get("requirement_id")): item for item in ledger["requirements"] if str(item.get("requirement_id")) in P10_SCOPE_IDS}
    if set(selected) != set(P10_SCOPE_IDS):
        raise RuntimeError("Progress 10 source ledger does not contain the complete authorized scope")
    ...
    if included_ids | deferred_ids != set(P10_SCOPE_IDS) or included_ids & deferred_ids:
        raise RuntimeError("Progress 10 milestone scope is not an exact disjoint partition")
    ...
    return {
        ...
        "milestone": "Progress 10 bounded OPS-004 backup, restore, disaster recovery, retention, deletion, preservation, and portability contingency",
        "scope_statement": template["scope_statement"],
        "authoritative_wording": MILESTONE_WORDING,
        "scope_requirement_ids": sorted(P10_SCOPE_IDS),
        "included_requirements": included,
        "deferred_requirements": deferred,
        "summary": {
            "scope_requirements_total": len(P10_SCOPE_IDS),
...
def _render_status(facts: dict[str, Any], counts: dict[str, int]) -> str:
    lines = [
        "# SIP v1.1.0 Implementation Status — Progress 10",
        "",
        MILESTONE_WORDING,
        "",
    ...
    lines += [
        ...
        "## Release posture",
        "",
        "Progress 10 is a source-bound bounded backup, restore, disaster-recovery, retention, deletion, preservation, portability-contingency, and conservative migration checkpoint. Progress 11 is unauthorized and production deployment and release remain NO-GO.",
        "",
...
def _render_resume(facts: dict[str, Any]) -> str:
    return "\n".join([
        ...
        "",
        "This continuation record is generated from `build/checkpoints/progress-10.json`.",
        "",
        f"- Branch: `{facts['branch']}`",
...
def _render_coverage(facts: dict[str, Any], ledger: dict[str, Any]) -> str:
    ...
    lines = [
        "# Requirements coverage report — Progress 10",
        "",
        f"Checkpoint: `{facts['checkpoint_id']}`",
        f"Commit: `{facts['commit']}`",
        f"Source root: `{facts['source_tree_root_sha256']}`",
    ...
    lines += [
        ...
        "",
        "- Progress 10 is delivered for independent True North milestone-closure review.",
        "- Progress 10 implementation was authorized; milestone closure remains pending independent True North review.",
        "- Progress 11 remains **unauthorized**.",
...
def _require_acceptance(report: dict[str, Any], *, commit: str, source_root: str) -> None:
    ...
    if report.get("checkpoint_id") != CHECKPOINT_ID or source.get("commit") != commit or source.get("source_tree_root_sha256") != source_root:
        raise RuntimeError("checkpoint acceptance report is not bound to Progress 10 source")
...
def build(*, evidence_root: Path, output: Path, branch: str, attestation_path: Path) -> dict[str, Any]:
    ...
    if python_tests < 300 or any(int(totals.get(key, 0)) for key in ("failures", "errors", "skipped")):
        raise RuntimeError("Python matrix does not satisfy the Progress 10 baseline")
    ...
    readiness = _load_json(evidence_root / "build/reports/release-readiness-progress-10.json")
    ...
    with tempfile.TemporaryDirectory(prefix="sip-progress-10-build-") as temporary:
        ...
        bundle = artifacts_dir / f"Spatial-Intelligence-Platform-v1.1.0-progress-10-{commit}.bundle"
        ...
        _write(stage / "build/checkpoints/progress-10.json", checkpoint_record)
        milestone = _milestone_scope(ledger=ledger, facts=facts)
        _write(stage / "MILESTONE_SCOPE_PROGRESS_10.json", milestone)
        ...
        latest = {
            ...
            "source_tree_root_sha256": source_root,
            "readiness": "blocked",
            "checkpoint_record": "build/checkpoints/progress-10.json",
            "progress_10_authorized": True,
            "progress_11_authorized": False,
            "production_authorized": False,
        }
        ...
        raw_paths: dict[str, list[str]] = {
            ...
            "demonstrations": [
                ...
                "build/evidence/demos/construction.json", "build/evidence/demos/liveforever.json", "build/evidence/demo-progress07-security.json", "build/evidence/demo-progress08-operations.json", "build/evidence/demo-progress10-recovery.json",
                ...
                "build/evidence/gates/demo-construction.log", "build/evidence/gates/demo-liveforever.log", "build/evidence/gates/demo-security.log", "build/evidence/gates/demo-operations.log", "build/evidence/gates/demo-recovery.log",
            ],
            ...
            "release_readiness": ["build/reports/release-readiness-progress-10.json", "build/reports/checkpoint-acceptance-gates.json", "build/reports/release-report.json", "build/evidence/gates/web-acceptance.log", "build/evidence/gates/release.log", "build/evidence/gates/release-mode.log"],
        }
        statuses = {
        ...
        report = "\n".join([
            "# Progress 10 Final Implementation Report",
            "",
            ...
            MILESTONE_WORDING,
            "",
            "## Bounded OPS-004 implementation",
            "",
            "Immutable recovery points, isolated restore, root-hash reconciliation, legal holds, dependency-aware purge, backup expiry, cryptographic erasure, preservation fixity, format migration, tenant offboarding, synthetic game days, portability contingency, and conservative legacy migration are implemented as the bounded Progress 10 OPS-004 milestone.",
            "",
            "Recovery records are immutable and digest-bound; tenant, project, time, residency, key, legal-hold, and anti-resurrection policy is enforced before restore or purge; writes reopen only after reconciliation; and production promotion remains fail-closed without executed external recovery evidence.",
            "",
            "All Progress 10 demonstrations use synthetic or non-sensitive fixtures. No confidential facility, critical-infrastructure, private-family, cloned voice, simulated likeness, or unapproved external-provider data is included.",
            "",
            ...
            "",
            "Append-only schema revision 0020_progress10_recovery_retention is included. All earlier migrations are preserved unchanged.",
            "",
            "Progress 10 is authorized only as this bounded development milestone. Progress 11 remains unauthorized and production deployment/release remains NO-GO.",
        ]) + "\n"
        (stage / "FINAL_IMPLEMENTATION_REPORT.md").write_text(report, encoding="utf-8")
        milestone_report = "\n".join([
            "# Progress 10 Milestone Report",
            "",
            f"Checkpoint: `{CHECKPOINT_ID}`",
            f"Commit: `{commit}`",
            f"Source root: `{source_root}`",
            ...
            MILESTONE_WORDING,
            "",
            f"- Bounded OPS-004 scope: {len(P10_SCOPE_IDS)} requirements, with every included/deferred item explicitly partitioned.",
            ...
            f"Python matrix: {python_tests} passed, 0 failed, 0 errors, 0 skipped.",
            "",
            "Progress 10 is delivered for independent True North milestone-closure review. Progress 11 remains unauthorized and production remains NO-GO.",
        ]) + "\n"
        (stage / "PROGRESS_10_MILESTONE_REPORT.md").write_text(milestone_report, encoding="utf-8")
        checkpoint_readme = "\n".join([
            "# SIP v1.1.0 Progress 10 checkpoint",
            "",
            ...
            "",
            "```bash",
            "python source/tools/verify_progress10_checkpoint.py Spatial-Intelligence-Platform-v1.1.0-progress-10.zip",
            "```",
            "",
            "Clone the included Git bundle:",
            "",
            "```bash",
            f"git clone -b {branch} artifacts/Spatial-Intelligence-Platform-v1.1.0-progress-10-{commit}.bundle <destination>",
            "```",
            "",
            "Production recovery certification, deployment, and release remain NO-GO.",
        ]) + "\n"
...
def main() -> None:
    ...
    parser.add_argument("--branch", default="progress-10-recovery")
