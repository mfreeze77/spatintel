#!/usr/bin/env python3
"""Run the Progress 09 clean-source acceptance sequence and retain source-bound gate evidence."""
...
def _acceptance_lock_path(root: Path = ROOT) -> Path:
    ...
    return Path(tempfile.gettempdir()) / f"sip-progress-09-acceptance-{identity}.lock"
...
def _exclusive_acceptance_lock():
    ...
    with lock_path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            raise RuntimeError("Progress 09 checkpoint acceptance is already running") from exc
        handle.seek(0)
        handle.truncate()
...
TARGET_REPORT_PATHS = {
    ...
    "demo-deployment": "build/evidence/demo-progress09-deployment.json",
...
def run(*, attestation_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "release_block_reason": "Progress 09 milestone closure requires independent True North review; Progress 10 and production remain unauthorized, and external validation gaps remain open.",
    }
    ...
    with _exclusive_acceptance_lock():
        ...
        "progress_10_authorized": False,
            ...
            "external_validation_gaps": [
                ...
                "Progress 09 deployment profiles and AWS/edge evidence are synthetic structural fixtures; no production cluster, cloud account, or physical edge node was deployed.",
            ...
            "next_action": "Submit the independently verified Progress 09 inner checkpoint and consolidated outer envelope for True North review; Progress 10 and production remain unauthorized.",
        }
        readiness_path = ROOT / "build/reports/release-readiness-progress-09.json"
