#!/usr/bin/env python3
"""Independently verify a SIP Progress 06-R2 checkpoint ZIP and all retained evidence."""
...
def _verify_accepted_base_ancestry(
    ...
    """Prove the checkpoint commit descends from the accepted Progress 06-R1 base."""
    ...
    with tempfile.TemporaryDirectory(prefix="sip-progress06-ancestry-") as temporary:
        ...
        if base_exists.returncode != 0:
            verification.fail("BASE_COMMIT_MISSING", "accepted Progress 06-R1 commit is absent from the Git bundle", bundle_relative)
            return
...
def _verify_predecessor(root: Path, verification: Verification) -> None:
    ...
    accepted = package_record.get("accepted_progress_06_r1_checkpoint")
    if not isinstance(accepted, dict):
        verification.fail("PREDECESSOR_PROGRESS06_MISSING", "accepted Progress 06-R1 record is absent", relative)
        return
    for key, expected in (
...
def _verify_scope_and_traceability(root: Path, verification: Verification, facts: dict[str, Any]) -> None:
    package_relative = "MILESTONE_SCOPE_PROGRESS_06_R2.json"
    package_scope = _read_json(root / package_relative, verification, code="MILESTONE_SCOPE_INVALID")
    source_scope = _read_json(root / "source/requirements/MILESTONE_SCOPE_PROGRESS_06_R2.json", verification, code="SOURCE_MILESTONE_SCOPE_INVALID")
    ...
    audit = _read_json(root / "source/requirements/progress-06-r2-traceability-audit.json", verification, code="TRACEABILITY_AUDIT_INVALID")
    ...
    if package_scope.get("authoritative_wording") != MILESTONE_WORDING:
        verification.fail("MILESTONE_WORDING", "authoritative wording differs from the authorized Progress 06-R2 scope", package_relative)
    ...
    if set(included) & set(deferred) or len(set(included) | set(deferred)) != EXPECTED_SCOPE_TOTAL:
        verification.fail("MILESTONE_PARTITION", "R2 milestone scope is not an exact 19-requirement partition", package_relative)
    ...
    if audit.get("status") != "passed_complete" or int(audit.get("finding_count", -1)) != 0:
        verification.fail("TRACEABILITY_AUDIT", "Progress 06-R2 traceability audit did not pass with zero findings", "source/requirements/progress-06-r2-traceability-audit.json")
    if int(audit.get("requirement_count", -1)) != EXPECTED_SCOPE_TOTAL:
        verification.fail("TRACEABILITY_SCOPE", "R2 traceability audit did not cover all 19 remediation requirements", "source/requirements/progress-06-r2-traceability-audit.json")
    if audit.get("progress_07_authorized") is not False or audit.get("production_authorized") is not False:
        verification.fail("TRACEABILITY_POSTURE", "traceability audit incorrectly authorizes a later phase", "source/requirements/progress-06-r2-traceability-audit.json")
    ...
    if len(audited) != EXPECTED_SCOPE_TOTAL:
        verification.fail("TRACEABILITY_RECORD_COUNT", "traceability audit record count differs", "source/requirements/progress-06-r2-traceability-audit.json")
    if {str(item.get("requirement_id")) for item in audited if isinstance(item, dict)} != set(included):
        verification.fail("TRACEABILITY_SCOPE_DRIFT", "traceability audit requirement set differs from the R2 milestone scope", "source/requirements/progress-06-r2-traceability-audit.json")
    for record in audited:
        if not isinstance(record, dict):
            verification.fail("TRACEABILITY_RECORD", "audit record must be an object", "source/requirements/progress-06-r2-traceability-audit.json")
            continue
        direct = record.get("direct_tests") if isinstance(record.get("direct_tests"), list) else []
        if not direct:
            verification.fail("TRACEABILITY_DIRECT_EVIDENCE", f"{record.get('requirement_id')} has no direct R2 evidence", "source/requirements/progress-06-r2-traceability-audit.json")
        for test in direct:
            if not isinstance(test, dict) or test.get("exists") is not True or test.get("declares_requirement_id") is not True or test.get("present_in_implementation_map") is not True:
                verification.fail("TRACEABILITY_DECLARATION", f"{record.get('requirement_id')} has invalid direct test evidence", "source/requirements/progress-06-r2-traceability-audit.json")
...
def _verify_acceptance(root: Path, verification: Verification, facts: dict[str, Any]) -> None:
    ...
    readiness_relative = "build/reports/release-readiness-progress-06-r2.json"
...
def _verify_migration(root: Path, verification: Verification) -> None:
    relative = "source/migrations/versions/0016_progress06_r2_truth_and_restricted_data.py"
    migration = root / relative
    if not migration.is_file():
        verification.fail("MIGRATION_0016_MISSING", "append-only Progress 06-R2 migration is missing", relative)
        return
    if migration.stat().st_size != EXPECTED_MIGRATION_BYTES or sha256_file(migration) != EXPECTED_MIGRATION_SHA256:
        verification.fail("MIGRATION_0016_HASH", "Progress 06-R2 migration bytes differ from the locked revision", relative)
    ...
    matched = [item for item in records if isinstance(item, dict) and item.get("path") == "migrations/versions/0016_progress06_r2_truth_and_restricted_data.py"]
    if len(matched) != 1:
        verification.fail("MIGRATION_0016_MANIFEST", "migration manifest does not contain exactly one 0016 record", "source/migrations/manifest.json")
    else:
        item = matched[0]
        if int(item.get("byte_count", -1)) != EXPECTED_MIGRATION_BYTES or item.get("sha256") != EXPECTED_MIGRATION_SHA256:
            verification.fail("MIGRATION_0016_MANIFEST_HASH", "migration manifest lock differs", "source/migrations/manifest.json")


def _verify_status_and_evidence(root: Path, verification: Verification, source_record: dict[str, Any] | None) -> None:
    checkpoint_relative = "build/checkpoints/progress-06-r2.json"
    ...
    if facts.get("checkpoint_id") != CHECKPOINT_ID or facts.get("branch") != "progress-06-r2-remediation":
    ...
    if int(facts.get("python_tests_passed", -1)) < MINIMUM_PYTHON_TESTS:
        verification.fail("CHECKPOINT_TEST_BASELINE", "Python test count is below the Progress 06-R2 minimum", checkpoint_relative)
    ...
    if checkpoint.get("milestone_wording") != MILESTONE_WORDING:
        verification.fail("CHECKPOINT_WORDING", "checkpoint wording differs from the authorized Progress 06-R2 scope", checkpoint_relative)
    if source_record:
    ...
    required_docs = (
        ...
        "FINAL_IMPLEMENTATION_REPORT.md", "PROGRESS_06_R2_REMEDIATION_REPORT.md",
    )
