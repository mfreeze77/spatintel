#!/usr/bin/env python3
"""Audit the exact bounded Progress 09 OPS-003 milestone against executable evidence."""
...
SCOPE = ROOT / "requirements/MILESTONE_SCOPE_PROGRESS_09.json"
AUDIT = ROOT / "requirements/progress-09-traceability-audit.json"
REQ = re.compile(r"\b[A-Z][A-Z0-9]+-\d{3}\b")
EXPECTED_BASE = "cc3c24ca3acff704144ed7d112566a2acaa88d79"
EXPECTED_INCLUDED = 17
EXPECTED_DEFERRED = 0
ALLOWED_PREFIXES = ("ARCDEP-", "OPSHYB-", "OPSAWS-")
...
def build() -> dict[str, Any]:
    ...
    if scope.get("authorized_epics") != ["OPS-003"]:
    ...
    if scope.get("progress_09_authorized") is not True or scope.get("progress_10_authorized") is not False or scope.get("production_authorized") is not False:
    ...
    return {
        "schema": "sip.progress09-traceability-audit/v1",
        "milestone": "Progress 09 bounded deployment profiles and production-shaped infrastructure",
...
def main() -> None:
    ...
    if args.check:
        if not AUDIT.is_file() or AUDIT.read_text(encoding="utf-8") != text:
            raise SystemExit("Progress 09 traceability audit drift detected; run tools/audit_progress09_traceability.py")
    else:
