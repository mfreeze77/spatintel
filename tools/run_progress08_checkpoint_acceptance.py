#!/usr/bin/env python3
"""Run the Progress 08 clean-source acceptance sequence and retain source-bound gate evidence."""
...
def _acceptance_lock_path(root: Path = ROOT) -> Path:
    ...
    return Path(tempfile.gettempdir()) / f"sip-progress-08-acceptance-{identity}.lock"
...
def _exclusive_acceptance_lock():
    ...
    with lock_path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            raise RuntimeError("Progress 08 checkpoint acceptance is already running") from exc
        handle.seek(0)
        handle.truncate()
...
def run(*, attestation_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "release_block_reason": "Progress 08 milestone closure requires independent True North review; Progress 09 and production remain unauthorized, and external validation gaps remain open.",
    }
...
def main() -> None:
    ...
    with _exclusive_acceptance_lock():
        ...
        readiness = {
            ...
            "next_action": "Submit the independently verified Progress 08 inner checkpoint and consolidated outer envelope for True North review; Progress 09 and production remain unauthorized.",
        }
        readiness_path = ROOT / "build/reports/release-readiness-progress-08.json"
