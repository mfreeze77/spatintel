#!/usr/bin/env python3
"""Independently verify a SIP Progress 07 checkpoint ZIP and all retained evidence."""
...
TOP_LEVEL = "Spatial-Intelligence-Platform-v1.1.0-progress-07"
EXPECTED_SPEC_SHA256 = "84b570464b98b1baf3107789af22edad2a14086e5420df35a961f5fb16980bb8"
EXPECTED_BASE_COMMIT = "600e3d81ffb47a88cc0a3041b7fdc7a5901a9fe8"
EXPECTED_BASE_ZIP_SHA256 = "c3070e7d30b94d9bf1e214d65f1079bf45222cf96a1a06b1d2e9f878bf1aeb2f"
EXPECTED_BASE_OUTER_SHA256 = "a8890dad7ba0f2bb9cf5a1a5466b65466c2b97eef6aeabd821b3e3c2c8aec61b"
EXPECTED_BASE_SOURCE_ROOT = "f6980edfe32e56d6e92d4495de01e7922e6b4f54adaa6d6e9efd81ab50e5ee68"
EXPECTED_MIGRATION_SHA256 = "613013161ba4d2c9e66ee4a85cd98464f416ee87abdacb8d981fcbc26d7c94e9"
EXPECTED_MIGRATION_BYTES = 27308
EXPECTED_SCOPE_TOTAL = 64
EXPECTED_INCLUDED = 61
EXPECTED_DEFERRED = 3
MINIMUM_PYTHON_TESTS = 300
MILESTONE_WORDING = (
    "Progress 07 implements and verifies the bounded OPS-001 security, encryption, audit, privacy, "
    "provider-governance, key-management, and supply-chain control layer defined by the enclosed milestone scope. "
    "Container and Apple artifact signing, executed vendor agreements, jurisdiction-specific legal evidence, "
    "deployed infrastructure, independent reviews, and production promotion remain external or deferred. "
    "Progress 08 is unauthorized and production remains NO-GO."
)
AUTHORIZED_EPICS = {"OPS-001"}
...
def _verify_accepted_base_ancestry(
    ...
    """Prove the checkpoint commit descends from the accepted Progress 06-R2 base."""
    ...
    if not bundle.is_file():
        return
    with tempfile.TemporaryDirectory(prefix="sip-progress07-ancestry-") as temporary:
        ...
        if base_exists.returncode != 0:
            verification.fail("BASE_COMMIT_MISSING", "accepted Progress 06-R2 commit is absent from the Git bundle", bundle_relative)
            return
        ...
        if ancestry.returncode != 0:
            verification.fail(
                ...
                "checkpoint commit does not descend from accepted Progress 06",
                bundle_relative,
            )
...
def _verify_predecessor(root: Path, verification: Verification) -> None:
    ...
    accepted = package_record.get("accepted_progress_06_r2_checkpoint")
    if not isinstance(accepted, dict):
        verification.fail("PREDECESSOR_PROGRESS06_MISSING", "accepted Progress 06-R2 record is absent", relative)
        return
    for key, expected in (
        ...
        if accepted.get(key) != expected:
            verification.fail("PREDECESSOR_PROGRESS06_MISMATCH", f"{key} differs from the accepted Progress 06-R2 base", relative)
...
def _verify_scope_and_traceability(root: Path, verification: Verification, facts: dict[str, Any]) -> None:
    package_relative = "MILESTONE_SCOPE_PROGRESS_07.json"
    package_scope = _read_json(root / package_relative, verification, code="MILESTONE_SCOPE_INVALID")
    source_scope = _read_json(root / "source/requirements/MILESTONE_SCOPE_PROGRESS_07.json", verification, code="SOURCE_MILESTONE_SCOPE_INVALID")
    ...
    audit = _read_json(root / "source/requirements/progress-07-traceability-audit.json", verification, code="TRACEABILITY_AUDIT_INVALID")
    ...
    if package_scope.get("authoritative_wording") != MILESTONE_WORDING:
        verification.fail("MILESTONE_WORDING", "authoritative wording differs from the authorized Progress 07 scope", package_relative)
    ...
    if package_scope.get("progress_07_authorized") is not True or package_scope.get("progress_08_authorized") is not False or package_scope.get("production_authorized") is not False:
        verification.fail("MILESTONE_POSTURE", "Progress 07 authorization, Progress 08 posture, or production posture is incorrect", package_relative)
    ...
    if set(included) & set(deferred) or len(set(included) | set(deferred)) != EXPECTED_SCOPE_TOTAL:
        verification.fail("MILESTONE_PARTITION", "Progress 07 milestone scope is not an exact 64-requirement partition", package_relative)
    ...
    if audit.get("status") != "passed_complete" or int(audit.get("finding_count", -1)) != 0:
        verification.fail("TRACEABILITY_AUDIT", "Progress 07 traceability audit did not pass with zero findings", "source/requirements/progress-07-traceability-audit.json")
    if int(audit.get("requirement_count", -1)) != EXPECTED_SCOPE_TOTAL:
        verification.fail("TRACEABILITY_SCOPE", "Progress 07 traceability audit did not cover all 64 scoped requirements", "source/requirements/progress-07-traceability-audit.json")
    if audit.get("progress_08_authorized") is not False or audit.get("production_authorized") is not False:
        verification.fail("TRACEABILITY_POSTURE", "traceability audit incorrectly authorizes Progress 08 or production", "source/requirements/progress-07-traceability-audit.json")
    ...
    if len(audited) != EXPECTED_SCOPE_TOTAL:
        verification.fail("TRACEABILITY_RECORD_COUNT", "traceability audit record count differs", "source/requirements/progress-07-traceability-audit.json")
    if {str(item.get("requirement_id")) for item in audited if isinstance(item, dict)} != (set(included) | set(deferred)):
        verification.fail("TRACEABILITY_SCOPE_DRIFT", "traceability audit requirement set differs from the Progress 07 milestone scope", "source/requirements/progress-07-traceability-audit.json")
    for record in audited:
        if not isinstance(record, dict):
            verification.fail("TRACEABILITY_RECORD", "audit record must be an object", "source/requirements/progress-07-traceability-audit.json")
            continue
        ...
        if requirement_id in included and not direct:
            verification.fail("TRACEABILITY_DIRECT_EVIDENCE", f"{record.get('requirement_id')} has no direct Progress 07 evidence", "source/requirements/progress-07-traceability-audit.json")
        for test in direct:
            if not isinstance(test, dict) or test.get("exists") is not True or test.get("declares_requirement_id") is not True or test.get("present_in_implementation_map") is not True:
                verification.fail("TRACEABILITY_DECLARATION", f"{record.get('requirement_id')} has invalid direct test evidence", "source/requirements/progress-07-traceability-audit.json")
...
def _verify_acceptance(root: Path, verification: Verification, facts: dict[str, Any]) -> None:
    ...
    if report.get("progress_07_authorized") is not True or report.get("progress_08_authorized") is not False or report.get("production_authorized") is not False:
    ...
    readiness_relative = "build/reports/release-readiness-progress-07.json"
    ...
    if readiness:
        ...
        if readiness.get("status") != "blocked" or readiness.get("production_authorized") is not False or readiness.get("progress_07_authorized") is not True or readiness.get("progress_08_authorized") is not False:
...
def _verify_migration(root: Path, verification: Verification) -> None:
    relative = "source/migrations/versions/0017_progress07_security_privacy_readiness.py"
    migration = root / relative
    if not migration.is_file():
        verification.fail("MIGRATION_0017_MISSING", "append-only Progress 07 migration is missing", relative)
        return
    if migration.stat().st_size != EXPECTED_MIGRATION_BYTES or sha256_file(migration) != EXPECTED_MIGRATION_SHA256:
        verification.fail("MIGRATION_0017_HASH", "Progress 07 migration bytes differ from the locked revision", relative)
    ...
    matched = [item for item in records if isinstance(item, dict) and item.get("path") == "migrations/versions/0017_progress07_security_privacy_readiness.py"]
    if len(matched) != 1:
        verification.fail("MIGRATION_0017_MANIFEST", "migration manifest does not contain exactly one 0017 record", "source/migrations/manifest.json")
    else:
        item = matched[0]
        if int(item.get("byte_count", -1)) != EXPECTED_MIGRATION_BYTES or item.get("sha256") != EXPECTED_MIGRATION_SHA256:
            verification.fail("MIGRATION_0017_MANIFEST_HASH", "migration manifest lock differs", "source/migrations/manifest.json")


def _verify_status_and_evidence(root: Path, verification: Verification, source_record: dict[str, Any] | None) -> None:
    checkpoint_relative = "build/checkpoints/progress-07.json"
    ...
    required = {
        ...
        "progress_07_authorized", "progress_08_authorized", "production_authorized",
    }
    for field in sorted(required - set(facts)):
        verification.fail("CHECKPOINT_FACT_MISSING", f"missing fact {field}", checkpoint_relative)
    if facts.get("checkpoint_id") != CHECKPOINT_ID or facts.get("branch") != "progress-07-bounded":
    ...
    if int(facts.get("python_tests_passed", -1)) < MINIMUM_PYTHON_TESTS:
        verification.fail("CHECKPOINT_TEST_BASELINE", "Python test count is below the Progress 07 minimum", checkpoint_relative)
    ...
    if facts.get("progress_07_authorized") is not True or facts.get("progress_08_authorized") is not False or facts.get("production_authorized") is not False:
        verification.fail("CHECKPOINT_POSTURE", "Progress 07 authorization, Progress 08 posture, or production posture is incorrect", checkpoint_relative)
    if checkpoint.get("milestone_wording") != MILESTONE_WORDING:
        verification.fail("CHECKPOINT_WORDING", "checkpoint wording differs from the authorized Progress 07 scope", checkpoint_relative)
    if source_record:
    ...
    if latest:
        ...
        if latest.get("progress_07_authorized") is not True or latest.get("progress_08_authorized") is not False or latest.get("production_authorized") is not False:
            verification.fail("LATEST_POSTURE", "latest record has incorrect Progress 07/08 or production posture", "build/release/latest.json")
    ...
    required_docs = (
        ...
        "FINAL_IMPLEMENTATION_REPORT.md", "PROGRESS_07_MILESTONE_REPORT.md",
    )
    for relative in required_docs:
        ...
        if "Progress 07" not in text or "Progress 08" not in text or not any(token in text for token in ("NO-GO", "blocked", "unauthorized", "not authorized")):
...
def verify_archive(archive_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="sip-progress07-verify-") as temporary:
