#!/usr/bin/env python3
"""Independently verify a SIP Progress 08 checkpoint ZIP and all retained evidence."""
...
TOP_LEVEL = "Spatial-Intelligence-Platform-v1.1.0-progress-08"
EXPECTED_SPEC_SHA256 = "84b570464b98b1baf3107789af22edad2a14086e5420df35a961f5fb16980bb8"
EXPECTED_BASE_COMMIT = "986c198a127616f8bf1d2379e9f9df5138dabc40"
EXPECTED_BASE_ZIP_SHA256 = "cf2ee7aedb50e4b44cea0850c3f958e57368907df43bdfb3d4c535a1b987e8d1"
EXPECTED_BASE_OUTER_SHA256 = "cb9e363ba0ee4f39d4602710fd3b6e786853daa1439bd69b596d5bda2283ce83"
EXPECTED_BASE_SOURCE_ROOT = "66b9e9f1a831874f5c9d654d07a97b9b79a4b38979a4d02dc4529a523c50472e"
EXPECTED_MIGRATION_SHA256 = "2d4a79656918e8b0eab057e3d865f3512c0408b4cbe21b5cc0b15a2b3f85b12f"
EXPECTED_MIGRATION_BYTES = 44262
EXPECTED_SCOPE_TOTAL = 40
EXPECTED_INCLUDED = 40
EXPECTED_DEFERRED = 0
MINIMUM_PYTHON_TESTS = 340
MILESTONE_WORDING = (
    "Progress 08 implements and verifies the bounded OPS-002 observability, SLO, cost, quota, resilience, performance-budget, and privacy-safe support layer defined by the enclosed milestone scope. "
    "Physical iOS performance, mounted-browser performance, approved GPU execution, deployed infrastructure, independent reviews, and production promotion remain external. "
    "Progress 09 is unauthorized and production remains NO-GO."
)
...
def _verify_accepted_base_ancestry(
    ...
    """Prove the checkpoint commit descends from the accepted Progress 07 base."""
    ...
    if not bundle.is_file():
        return
    with tempfile.TemporaryDirectory(prefix="sip-progress08-ancestry-") as temporary:
        ...
        if base_exists.returncode != 0:
            verification.fail("BASE_COMMIT_MISSING", "accepted Progress 07 commit is absent from the Git bundle", bundle_relative)
            return
        ...
        if ancestry.returncode != 0:
            verification.fail(
                ...
                "checkpoint commit does not descend from accepted Progress 07",
                bundle_relative,
            )
...
def _verify_predecessor(root: Path, verification: Verification) -> None:
    ...
    accepted = package_record.get("accepted_progress_07_checkpoint")
    if not isinstance(accepted, dict):
        verification.fail("PREDECESSOR_PROGRESS07_MISSING", "accepted Progress 07 record is absent", relative)
        return
    for key, expected in (
        ...
        if accepted.get(key) != expected:
            verification.fail("PREDECESSOR_PROGRESS07_MISMATCH", f"{key} differs from the accepted Progress 07 base", relative)
...
def _verify_scope_and_traceability(root: Path, verification: Verification, facts: dict[str, Any]) -> None:
    package_relative = "MILESTONE_SCOPE_PROGRESS_08.json"
    package_scope = _read_json(root / package_relative, verification, code="MILESTONE_SCOPE_INVALID")
    source_scope = _read_json(root / "source/requirements/MILESTONE_SCOPE_PROGRESS_08.json", verification, code="SOURCE_MILESTONE_SCOPE_INVALID")
    ...
    audit = _read_json(root / "source/requirements/progress-08-traceability-audit.json", verification, code="TRACEABILITY_AUDIT_INVALID")
    ...
    if package_scope.get("authoritative_wording") != MILESTONE_WORDING:
        verification.fail("MILESTONE_WORDING", "authoritative wording differs from the authorized Progress 08 scope", package_relative)
    ...
    if package_scope.get("progress_08_authorized") is not True or package_scope.get("progress_09_authorized") is not False or package_scope.get("production_authorized") is not False:
        verification.fail("MILESTONE_POSTURE", "Progress 08, Progress 09, or production posture is incorrect", package_relative)
    ...
    if set(included) & set(deferred) or len(set(included) | set(deferred)) != EXPECTED_SCOPE_TOTAL:
        verification.fail("MILESTONE_PARTITION", "Progress 08 milestone scope is not an exact 40-requirement partition", package_relative)
    ...
    if audit.get("status") != "passed_complete" or int(audit.get("finding_count", -1)) != 0:
        verification.fail("TRACEABILITY_AUDIT", "Progress 08 traceability audit did not pass with zero findings", "source/requirements/progress-08-traceability-audit.json")
    if int(audit.get("requirement_count", -1)) != EXPECTED_SCOPE_TOTAL:
        verification.fail("TRACEABILITY_SCOPE", "Progress 08 traceability audit did not cover all 40 scoped requirements", "source/requirements/progress-08-traceability-audit.json")
    if audit.get("progress_09_authorized") is not False or audit.get("production_authorized") is not False:
        verification.fail("TRACEABILITY_POSTURE", "traceability audit incorrectly authorizes Progress 08 or production", "source/requirements/progress-08-traceability-audit.json")
    ...
    if len(audited) != EXPECTED_SCOPE_TOTAL:
        verification.fail("TRACEABILITY_RECORD_COUNT", "traceability audit record count differs", "source/requirements/progress-08-traceability-audit.json")
    if {str(item.get("requirement_id")) for item in audited if isinstance(item, dict)} != (set(included) | set(deferred)):
        verification.fail("TRACEABILITY_SCOPE_DRIFT", "traceability audit requirement set differs from the Progress 08 milestone scope", "source/requirements/progress-08-traceability-audit.json")
    for record in audited:
        if not isinstance(record, dict):
            verification.fail("TRACEABILITY_RECORD", "audit record must be an object", "source/requirements/progress-08-traceability-audit.json")
            continue
        ...
        if requirement_id in included and not direct:
            verification.fail("TRACEABILITY_DIRECT_EVIDENCE", f"{record.get('requirement_id')} has no direct Progress 08 evidence", "source/requirements/progress-08-traceability-audit.json")
        for test in direct:
            if not isinstance(test, dict) or test.get("exists") is not True or test.get("declares_requirement_id") is not True or test.get("present_in_implementation_map") is not True:
                verification.fail("TRACEABILITY_DECLARATION", f"{record.get('requirement_id')} has invalid direct test evidence", "source/requirements/progress-08-traceability-audit.json")
...
def _verify_acceptance(root: Path, verification: Verification, facts: dict[str, Any]) -> None:
    ...
    if report.get("progress_08_authorized") is not True or report.get("progress_09_authorized") is not False or report.get("production_authorized") is not False:
    ...
    readiness_relative = "build/reports/release-readiness-progress-08.json"
    ...
    if readiness:
        ...
        if readiness.get("status") != "blocked" or readiness.get("production_authorized") is not False or readiness.get("progress_08_authorized") is not True or readiness.get("progress_09_authorized") is not False:
        ...
        if "True North" not in next_action or "Progress 08" not in next_action:
...
def _verify_migration(root: Path, verification: Verification) -> None:
    relative = "source/migrations/versions/0018_progress08_observability_cost_support.py"
    migration = root / relative
    if not migration.is_file():
        verification.fail("MIGRATION_0018_MISSING", "append-only Progress 08 migration is missing", relative)
        return
    if migration.stat().st_size != EXPECTED_MIGRATION_BYTES or sha256_file(migration) != EXPECTED_MIGRATION_SHA256:
        verification.fail("MIGRATION_0018_HASH", "Progress 08 migration bytes differ from the locked revision", relative)
    ...
    matched = [item for item in records if isinstance(item, dict) and item.get("path") == "migrations/versions/0018_progress08_observability_cost_support.py"]
    if len(matched) != 1:
        verification.fail("MIGRATION_0018_MANIFEST", "migration manifest does not contain exactly one 0018 record", "source/migrations/manifest.json")
    else:
        item = matched[0]
        if int(item.get("byte_count", -1)) != EXPECTED_MIGRATION_BYTES or item.get("sha256") != EXPECTED_MIGRATION_SHA256:
            verification.fail("MIGRATION_0018_MANIFEST_HASH", "migration manifest lock differs", "source/migrations/manifest.json")


def _verify_status_and_evidence(root: Path, verification: Verification, source_record: dict[str, Any] | None) -> None:
    checkpoint_relative = "build/checkpoints/progress-08.json"
    ...
    required = {
        ...
        "progress_08_authorized", "progress_09_authorized", "production_authorized",
    }
    for field in sorted(required - set(facts)):
        verification.fail("CHECKPOINT_FACT_MISSING", f"missing fact {field}", checkpoint_relative)
    if facts.get("checkpoint_id") != CHECKPOINT_ID or facts.get("branch") != "progress-08-observability":
    ...
    if int(facts.get("python_tests_passed", -1)) < MINIMUM_PYTHON_TESTS:
        verification.fail("CHECKPOINT_TEST_BASELINE", "Python test count is below the Progress 08 minimum", checkpoint_relative)
    ...
    if facts.get("progress_08_authorized") is not True or facts.get("progress_09_authorized") is not False or facts.get("production_authorized") is not False:
        verification.fail("CHECKPOINT_POSTURE", "Progress 08, Progress 09, or production posture is incorrect", checkpoint_relative)
    if checkpoint.get("milestone_wording") != MILESTONE_WORDING:
        verification.fail("CHECKPOINT_WORDING", "checkpoint wording differs from the authorized Progress 08 scope", checkpoint_relative)
    if source_record:
    ...
    if latest:
        ...
        if latest.get("progress_08_authorized") is not True or latest.get("progress_09_authorized") is not False or latest.get("production_authorized") is not False:
            verification.fail("LATEST_POSTURE", "latest record has incorrect Progress 08/09 or production posture", "build/release/latest.json")
    ...
    required_docs = (
        ...
        "FINAL_IMPLEMENTATION_REPORT.md", "PROGRESS_08_MILESTONE_REPORT.md",
    )
    for relative in required_docs:
        ...
        if "Progress 08" not in text or "Progress 09" not in text or not any(token in text for token in ("NO-GO", "blocked", "unauthorized", "not authorized")):
...
def verify_archive(archive_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="sip-progress08-verify-") as temporary:
