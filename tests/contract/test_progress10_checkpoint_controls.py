...
def test_progress10_predecessor_record_identifies_exact_accepted_progress09_checkpoint() -> None:
    """CONTROL: Progress 10 provenance is bound to the accepted Progress 09 commit and packages."""

    record = _json("PREDECESSOR_CHECKPOINT.json")["accepted_progress_09_checkpoint"]
    assert record["checkpoint_id"] == "sip-v1.1.0-progress-09"
    assert record["branch"] == "progress-09-deployment"
...
def test_progress10_scope_traceability_and_posture_are_exact_and_fail_closed() -> None:
    """CONTROL: the authorized recovery and retention scope is exact while later phases remain denied."""

    scope = _json("requirements/MILESTONE_SCOPE_PROGRESS_10.json")
    audit = _json("requirements/progress-10-traceability-audit.json")
    ...
    assert scope["authorized_epics"] == ["OPS-004"]
    assert scope["scope_statement"].startswith("Progress 10 is limited to OPS-004")
    assert "Progress 11" in scope["scope_statement"] and "production promotion" in scope["scope_statement"]
    ...
    assert all(identifier.startswith(("ARCRES-", "DATRET-", "PLTIO-", "LIFPRESV-", "OPSDR-", "SIPMIG-")) for identifier in identifiers)
    assert scope["progress_10_authorized"] is True
    assert scope["progress_11_authorized"] is False
    ...
    assert audit["progress_10_authorized"] is True
    assert audit["progress_11_authorized"] is False
...
def test_progress10_append_only_migration_is_integrity_locked() -> None:
    """REQ: DATDB-002 Progress 10 schema additions are append-only and byte-locked at migration 0020."""

    path = ROOT / "migrations/versions/0020_progress10_recovery_retention.py"
    ...
    assert "down_revision = '0019_progress09_deployment_profiles'" in source


def test_progress10_generated_service_and_event_contracts_are_owned_by_recovery_control() -> None:
    """CONTROL: generated contracts preserve the recovery-control boundary."""

    service = _json("services/recovery-control/service.json")
    assert service["service_name"] == "recovery-control"
    operation_paths = {item["path"] for item in service["operations"]}
    assert operation_paths
    openapi = _json("schemas/openapi/recovery-control.openapi.json")
    ...
    assert any("/recovery/points" in path for path in public_operations)
    assert any("/recovery/restores" in path for path in public_operations)
    assert any("/deletion-graphs" in path for path in public_operations)
    catalog = _json("schemas/events/event-catalog.json")
    owned = [item for item in catalog["events"] if item.get("owner") == "recovery-control"]
    assert len(owned) >= 20
    assert {item["type"] for item in owned} >= {
        "recovery.point_created",
        "recovery.point_verified",
        "recovery.restore_completed",
        "deletion.graph_planned",
        "deletion.purge_completed",
        "migration.legacy_validated",
    }


def test_progress10_acceptance_cli_rejects_concurrent_evidence_writers() -> None:
    """CONTROL: concurrent Progress 10 acceptance cannot overwrite immutable gate snapshots."""

    from tools.run_progress10_checkpoint_acceptance import _acceptance_lock_path
    ...
    with lock_path.open("a+", encoding="utf-8") as handle:
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "-c",
                "from tools.run_progress10_checkpoint_acceptance import _exclusive_acceptance_lock; ctx=_exclusive_acceptance_lock(); ctx.__enter__()",
            ],
            cwd=ROOT,
...
def test_progress10_checkpoint_builder_uses_separate_direct_security_evidence() -> None:
