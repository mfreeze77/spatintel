---
spec_id: CAP-FUT
title: "Future Sensor and Platform Extensions"
version: 1.1.0
...
normative: true
---
# Future Sensor and Platform Extensions

    ## 1. Purpose

    Defines compatible extension paths for external LiDAR, 360 cameras, drones, thermal, UWB, GNSS/RTK, depth cameras, scanners, and robotics.
    ...
    - New sensors enter through clock, calibration, coordinate-frame, asset, observation, and quality interfaces.
- The canonical package supports opaque extensions while requiring converters for authoritative use.
- A sensor's presence does not automatically increase authority; its calibration and workflow determine authority.
- Aerial and public-space capture require additional safety, legal, and privacy profiles.
...
| ---------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CAPFUT-001 | P0       | A sensor adapter shall document units, axes, timestamp semantics, calibration, uncertainty, invalid values, and operating envelope. | Controlled verification |
| CAPFUT-002 | P0       | The fusion pipeline shall be able to exclude a source without rebuilding unrelated derivatives.                                     | Controlled verification |
| CAPFUT-003 | P1       | External survey imports shall preserve source coordinate reference systems and transformation evidence.                             | Controlled verification |
| CAPFUT-004 | P1       | Thermal and other non-visible imagery shall be stored as registered observations, not painted into RGB truth by default.            | Controlled verification |
| CAPFUT-005 | P1       | Drone and robotics integrations shall remain read-only with respect to actuation in the initial platform.                           | Controlled verification |
    ...
    - A versioned `cap_fut_manifest` recording identity, timestamps, ownership, state, and provenance.
