---
spec_id: CAP-STRAT
title: "Capture Strategy and Source Abstraction"
version: 1.1.0
...
normative: true
---
# Capture Strategy and Source Abstraction

    ## 1. Purpose

    Defines the capture-source interface and how first-party, Polycam, ordinary video, image sets, and future sensors enter one pipeline.
    ...
    - The Canonical Spatial Capture Package is the only reconstruction input contract.
- Source adapters preserve native data and document every conversion.
- Capture profiles optimize for construction reference, verified measurement support, memory presence, fast walkthrough, or object detail.
- A session may combine multiple devices and return visits through explicit coordinate relationships.
...
| ------------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CAPSTRAT-001 | P0       | Every adapter shall output the same minimum manifest or declare unsupported fields explicitly.                                                     | Controlled verification |
| CAPSTRAT-002 | P0       | The ingest service shall retain the source-native package alongside normalized data when policy permits.                                           | Controlled verification |
| CAPSTRAT-003 | P1       | Capture profiles shall specify required sensors, resolution, frame rate, operator pattern, expected range, quality thresholds, and storage budget. | Controlled verification |
| CAPSTRAT-004 | P1       | The app shall not claim a profile's target accuracy until calibration and field validation demonstrate it.                                         | Controlled verification |
| CAPSTRAT-005 | P1       | A source adapter shall be testable against fixed fixtures without access to the original application.                                              | Controlled verification |
    ...
    - A versioned `cap_strat_manifest` recording identity, timestamps, ownership, state, and provenance.
