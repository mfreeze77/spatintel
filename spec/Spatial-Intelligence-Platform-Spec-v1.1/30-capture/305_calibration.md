---
spec_id: CAP-CAL
title: "Calibration and Known-Scale Controls"
version: 1.1.0
...
normative: true
---
# Calibration and Known-Scale Controls

    ## 1. Purpose

    Defines factory metadata, intrinsic/extrinsic calibration, validation targets, drift checks, fiducials, control points, and verified dimensions.
    ...
    - Factory device calibration is recorded but not assumed sufficient for every accuracy claim.
- Project profiles may require a calibration check at start and end of capture.
- Known scale can come from coded targets, scale bars, surveyed control, or accountable field dimensions.
- Calibration records are immutable and referenced by observations and pipeline runs.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| CAPCAL-001 | P0       | A calibration record shall state method, equipment/target, environment, operator, date, software, residuals, units, and validity criteria. | Controlled verification |
| CAPCAL-002 | P0       | The app shall reject or warn when a required calibration check fails.                                                                      | Automated test          |
| CAPCAL-003 | P0       | Control points shall include coordinate frame, uncertainty, method, verifier, and evidence.                                                | Controlled verification |
| CAPCAL-004 | P1       | Verified dimensions used as optimization constraints shall retain endpoints, measurement tool, uncertainty, and responsible person.        | Controlled verification |
| CAPCAL-005 | P1       | Calibration expiry or device/OS/camera changes shall trigger revalidation according to profile policy.                                     | Controlled verification |
| CAPCAL-006 | P1       | The optimizer shall report how strongly control constraints changed the unconstrained solution.                                            | Controlled verification |
    ...
    - A versioned `cap_cal_manifest` recording identity, timestamps, ownership, state, and provenance.
