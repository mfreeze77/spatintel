---
spec_id: CAP-TIME
title: "Timestamping and Sensor Synchronization"
version: 1.1.0
...
# Timestamping and Sensor Synchronization
    ...
    Defines clock domains, timestamp conversion, interpolation, latency estimation, rolling shutter, and cross-device synchronization.
    ...
    - Native timestamps are never discarded after conversion to a common timeline.
- Synchronization quality is represented as uncertainty and method.
- Cross-device sessions may use visual/audio events, network time, GNSS time, or physical sync markers.
- Frame extraction from video preserves source presentation/decode timestamps and extraction parameters.
...
| ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CAPTIME-001 | P0       | Every timestamped stream shall declare clock identifier, units, epoch/zero, monotonicity, and conversion to session time. | Controlled verification |
| CAPTIME-002 | P0       | Interpolated poses or IMU values shall be labeled and include maximum temporal gap.                                       | Controlled verification |
| CAPTIME-003 | P1       | The pipeline shall reject synchronization gaps beyond profile limits for tightly coupled fusion.                          | Automated test          |
| CAPTIME-004 | P1       | Audio and transcript anchors shall preserve sample-accurate or bounded time mapping to source media.                      | Controlled verification |
| CAPTIME-005 | P1       | Cross-device offset and drift estimates shall include residual and confidence.                                            | Controlled verification |
    ...
    - A versioned `cap_time_manifest` recording identity, timestamps, ownership, state, and provenance.
