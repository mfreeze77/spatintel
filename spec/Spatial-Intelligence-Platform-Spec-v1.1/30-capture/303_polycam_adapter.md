---
spec_id: CAP-POLY
title: "Polycam Raw Export Adapter"
version: 1.1.0
...
normative: true
---
# Polycam Raw Export Adapter

    ## 1. Purpose

    Defines ingestion of Polycam raw LiDAR/Room captures through the MIT-licensed polyform format without depending on proprietary internals.
    ...
    - The adapter consumes documented raw-export files and preserves the unmodified archive.
- Optimized poses and corrected images are used only when present and labeled with their source.
- ARKit poses remain available as an independent observation stream.
- Polycam mesh is a source geometry asset, not automatically the authoritative SIP metric mesh.
...
| ----------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CAPPOLY-001 | P0       | The adapter shall validate `raw.glb`, media, `mesh_info.json`, camera JSON, depth, confidence, and corrected variants independently.         | Automated test          |
| CAPPOLY-002 | P0       | Missing corrected poses shall not be synthesized or mislabeled as Polycam optimization output.                                               | Controlled verification |
| CAPPOLY-003 | P0       | The adapter shall map Polycam's right-handed gravity-aligned coordinate convention into a named source frame before any SIP transform.       | Controlled verification |
| CAPPOLY-004 | P1       | Depth resolution, units, invalid encoding, and confidence interpretation shall be recorded from the export metadata or marked unknown.       | Controlled verification |
| CAPPOLY-005 | P1       | The adapter shall produce a conversion report listing accepted, skipped, malformed, and unmatched files.                                     | Controlled verification |
| CAPPOLY-006 | P1       | Regression fixtures shall cover small optimized captures, large ARKit-only captures, missing depth, partial exports, and corrupted archives. | Controlled verification |
    ...
    - A versioned `cap_poly_manifest` recording identity, timestamps, ownership, state, and provenance.
