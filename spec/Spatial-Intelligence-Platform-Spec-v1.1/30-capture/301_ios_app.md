---
spec_id: CAP-IOS
title: "First-Party iOS Capture Application"
version: 1.1.0
...
normative: true
---
# First-Party iOS Capture Application

    ## 1. Purpose

    Defines the Swift application architecture, state machine, persistence, live feedback, session management, and export behavior.
    ...
    - SwiftUI owns application flow; ARKit/RealityKit/AVFoundation/CoreMotion own sensor acquisition through isolated modules.
- Capture state is persisted after every accepted frame batch and mesh-anchor update.
- The app shows a simple operator mode and an expert diagnostic overlay.
- The app can run local-only and exports a signed package through Files, local network, cable workflow, or resumable server upload.
...
| ---------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CAPIOS-001 | P0       | The app shall implement states for setup, calibration check, ready, capturing, paused, tracking recovery, segment finalization, session finalization, upload, verified, and failed-recoverable. | Controlled verification |
| CAPIOS-002 | P0       | The app shall capture RGB/depth/pose/intrinsics/IMU timestamps without blocking the AR session render loop.                                                                                     | Controlled verification |
| CAPIOS-003 | P0       | The app shall persist an append-only journal sufficient to recover after crash or device restart.                                                                                               | Automated test          |
| CAPIOS-004 | P1       | Thermal, storage, battery, and memory pressure shall trigger controlled quality degradation or pause with user explanation.                                                                     | Controlled verification |
| CAPIOS-005 | P1       | The app shall support project policy bundles that disable audio, location, cloud transfer, or particular sensors.                                                                               | Controlled verification |
| CAPIOS-006 | P1       | The app shall never delete local originals until package integrity and destination receipt are verified and the user or policy authorizes purge.                                                | Controlled verification |
    ...
    - A versioned `cap_ios_manifest` recording identity, timestamps, ownership, state, and provenance.
