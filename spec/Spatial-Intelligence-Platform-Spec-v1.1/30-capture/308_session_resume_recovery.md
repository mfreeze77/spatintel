---
spec_id: CAP-REC
title: "Capture Persistence, Resume, and Recovery"
version: 1.1.0
...
normative: true
---
# Capture Persistence, Resume, and Recovery

    ## 1. Purpose

    Defines crash safety, session journals, segment checkpoints, interrupted AR sessions, device storage pressure, and recovery tools.
    ...
    - Accepted data is append-only during capture.
- Recovery rebuilds indexes from the journal rather than trusting an incomplete summary.
- A relocalized continuation creates an explicit transform or a new segment requiring later registration.
- Package repair never modifies original bytes without preserving the damaged source and repair report.
...
| ---------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CAPREC-001 | P0       | The app shall fsync or otherwise durably commit finalized asset and journal records at bounded intervals.                           | Controlled verification |
| CAPREC-002 | P0       | On restart, the app shall verify recent hashes and offer resume, finalize-partial, export-for-repair, or discard with confirmation. | Automated test          |
| CAPREC-003 | P1       | Tracking loss shall not append poses as if they were valid world observations.                                                      | Controlled verification |
| CAPREC-004 | P1       | Low storage shall stop before filesystem exhaustion and preserve an exportable package.                                             | Controlled verification |
| CAPREC-005 | P1       | Repair tools shall create a new package revision with explicit lineage to the original.                                             | Controlled verification |
    ...
    - A versioned `cap_rec_manifest` recording identity, timestamps, ownership, state, and provenance.
