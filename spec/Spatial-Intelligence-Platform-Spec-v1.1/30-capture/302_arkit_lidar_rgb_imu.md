---
spec_id: CAP-SENS
title: "ARKit, LiDAR, RGB, Mesh, and IMU Acquisition"
version: 1.1.0
...
normative: true
---
# ARKit, LiDAR, RGB, Mesh, and IMU Acquisition

    ## 1. Purpose

    Defines synchronized sensor acquisition, metadata preservation, coordinate semantics, confidence interpretation, and sampling policy.
    ...
    - ARKit world transforms are stored exactly as provided plus an explicit transform convention record.
- Depth and confidence remain linked to the image and calibration used to interpret them.
- Mesh anchors are versioned observations, not treated as a final global mesh.
- IMU data is retained in its native clock/domain with synchronization estimates.
...
| ----------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CAPSENS-001 | P0       | Each frame record shall include timestamps, image asset, depth asset when present, confidence asset, camera transform, intrinsics, image resolution, tracking state, exposure, and orientation metadata. | Controlled verification |
| CAPSENS-002 | P0       | The capture layer shall preserve invalid depth and confidence values without converting them into zero-distance surfaces.                                                                                | Controlled verification |
| CAPSENS-003 | P0       | ARKit relocalization, world-origin changes, interruptions, and limited-tracking reasons shall create explicit events.                                                                                    | Automated test          |
| CAPSENS-004 | P1       | Mesh-anchor add/update/remove operations shall be journaled with identifiers and transforms.                                                                                                             | Controlled verification |
| CAPSENS-005 | P1       | Sensor sampling shall use bounded queues and record dropped frames with cause.                                                                                                                           | Controlled verification |
| CAPSENS-006 | P1       | Device model, OS, app build, sensor availability, and calibration-relevant camera identity shall be stored in the package manifest.                                                                      | Controlled verification |
    ...
    - A versioned `cap_sens_manifest` recording identity, timestamps, ownership, state, and provenance.
