---
spec_id: GOV-LIC
title: "Dependency, Dataset, and Model Governance"
version: 1.1.0
...
normative: true
---
# Dependency, Dataset, and Model Governance

    ## 1. Purpose

    Prevents code, weights, datasets, hosted APIs, or generated outputs from entering commercial workflows without evidence of permitted use.
    ...
    - Code, checkpoint, training/fine-tuning data, evaluation data, hosted service terms, and output restrictions are separate review objects.
- A repository license badge does not establish a checkpoint license.
- Models execute only when a signed model manifest is in an allowed deployment state.
- Noncommercial and incompatible copyleft components are excluded from the default proprietary distribution unless separately approved and isolated.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| GOVLIC-001 | P0       | Each model manifest shall record provider, model name, revision, checkpoint hash, source URLs, license documents, permitted uses, prohibited uses, geographic or customer restrictions, approver, and expiry/review date. | Automated test          |
| GOVLIC-002 | P0       | The scheduler shall reject production jobs whose model manifest is research-only, expired, revoked, or incomplete.                                                                                                        | Automated test          |
| GOVLIC-003 | P0       | Container builds shall produce an SBOM and license inventory including optional runtime downloads.                                                                                                                        | Controlled verification |
| GOVLIC-004 | P1       | The system shall prevent an adapter from downloading an unapproved checkpoint or segmentation model at first run.                                                                                                         | Automated test          |
| GOVLIC-005 | P1       | Dataset manifests shall document consent/provenance, license, transformations, retention, and whether outputs may be commercialized.                                                                                      | Controlled verification |
| GOVLIC-006 | P1       | A model replacement shall run shadow benchmarks and cannot inherit the prior model's approval automatically.                                                                                                              | Controlled verification |
    ...
    - A versioned `gov_lic_manifest` recording identity, timestamps, ownership, state, and provenance.
...
## 15. Version 1.1 spatial-provider governance

The manifest system extends to surface providers, browser/manual tools, renderers, file converters, geometry libraries, CUDA extensions, model checkpoints, datasets, sample assets, hosted terms, output rights, and transitive submodules. A permissive top-level badge does not approve the complete processing path.

| ID | Priority | Requirement | Verification |
|---|---|---|---|
| GOVLIC-007 | P0 | Every surface/conversion provider shall record exact code/service version, dependencies, models, datasets, terms, output rights, deployment, data classes, purposes, and expiration. | Manifest test |
| GOVLIC-008 | P0 | A top-level license shall not automatically approve submodules, checkpoints, datasets, binaries, samples, hosted processing, or outputs. | License audit |
| GOVLIC-009 | P0 | Manual/hosted provider admission shall fail closed when processing locality, retention, training use, security, or commercial permission is unknown for non-public data. | Governance test |
| GOVLIC-010 | P1 | Provider replacement shall require new technical and legal approval and shall not inherit the predecessor's production status. | Replacement test |
| GOVLIC-011 | P1 | Runtime and build systems shall prohibit undeclared provider/model/dependency downloads. | Supply-chain test |
| GOVLIC-012 | P1 | Historical derivatives shall retain the exact approval evidence and terms snapshot applicable at creation. | Provenance audit |
