...
# Document Control and Authority
    ...
    Defines ownership, precedence, versioning, approval, waiver, and retirement of the SIP specification and its implementation evidence.
    ...
    - The master specification and normative module documents are authoritative; examples and research notes are informative unless promoted.
- Schema, API, event, coordinate, consent, and authority-class changes require explicit compatibility review.
- Waivers have owners, compensating controls, expiry dates, and cannot relax truth-labeling or legal/license restrictions.
- Released specification bundles are immutable and identified by SHA-256.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| GOVDOC-001 | P0       | Every normative document shall carry a unique specification identifier, version, status, update date, category, and normative flag.                    | Controlled verification |
| GOVDOC-002 | P0       | The release process shall produce a manifest containing every file path, byte count, word count, and SHA-256 digest.                                   | Controlled verification |
| GOVDOC-003 | P1       | A breaking change shall identify affected schemas, APIs, stored data, exports, clients, tests, and migration steps.                                    | Automated test          |
| GOVDOC-004 | P1       | A waiver shall be rejected when it would cause unlicensed model use, unconsented biometric use, or presentation of generated content as verified fact. | Automated test          |
| GOVDOC-005 | P1       | Superseded bundles shall remain retrievable for audit and migration testing.                                                                           | Controlled verification |
    ...
    - A versioned `gov_doc_manifest` recording identity, timestamps, ownership, state, and provenance.
