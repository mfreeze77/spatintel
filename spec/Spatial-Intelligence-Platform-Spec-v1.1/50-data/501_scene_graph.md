---
spec_id: DAT-SCENE
title: "Persistent Semantic Scene Graph"
version: 1.1.0
...
# Persistent Semantic Scene Graph
    ...
    Defines stable places, entities, revisions, relationships, anchors, classifications, and authority across time.
    ...
    - Stable entities outlive geometry and scene revisions.
- Entity revisions have validity intervals and scene-commit membership.
- Relationships are typed, directed where meaningful, temporal, and evidence-backed.
- The ontology supports construction and memory without forcing unrelated fields onto every entity.
...
| ------------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| DATSCENE-001 | P0       | Every entity shall have a stable UUID, tenant/project scope, type, lifecycle status, and creation provenance.                                         | Controlled verification |
| DATSCENE-002 | P0       | An entity revision shall identify the scene commit or temporal validity, labels, properties, geometry references, anchors, permissions, and evidence. | Controlled verification |
| DATSCENE-003 | P0       | Relationships shall include type, endpoints, validity, confidence/source class, and provenance.                                                       | Controlled verification |
| DATSCENE-004 | P1       | Entity identity merges and splits shall be reversible and audited.                                                                                    | Controlled verification |
| DATSCENE-005 | P1       | Unknown or user-defined types shall use namespaced extensions without losing base behavior.                                                           | Controlled verification |
| DATSCENE-006 | P1       | Graph queries shall apply authorization before returning neighboring nodes or counts.                                                                 | Controlled verification |
    ...
    - A versioned `dat_scene_manifest` recording identity, timestamps, ownership, state, and provenance.
