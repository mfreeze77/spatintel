---
spec_id: DAT-ASSET
title: "Immutable Asset Storage and Content Addressing"
version: 1.1.0
...
normative: true
---
# Immutable Asset Storage and Content Addressing

    ## 1. Purpose

    Defines hashing, chunking, deduplication, encryption, multipart transfer, integrity verification, storage tiers, and garbage collection.
    ...
    - SHA-256 is the baseline content identity; future algorithms can be namespaced.
- Encryption identity and content identity are separated so re-encryption does not change logical asset identity.
- Cross-tenant physical deduplication must not leak existence or access.
- Garbage collection requires reference, retention, legal-hold, and backup analysis.
...
| ------------ | -------- | --------------------------------------------------------------------------------------------------------- | ----------------------- |
| DATASSET-001 | P0       | Assets shall be hashed before or during upload and verified after durable storage.                        | Automated test          |
| DATASSET-002 | P0       | Multipart uploads shall record chunk hashes and be safely resumable.                                      | Automated test          |
| DATASSET-003 | P0       | An asset shall not be published until size and hash match the manifest.                                   | Automated test          |
| DATASSET-004 | P1       | Storage-class transitions shall preserve integrity and retrieval guarantees required by retention policy. | Controlled verification |
| DATASSET-005 | P1       | Corruption detection shall trigger quarantine, replica comparison, restore, and incident/audit records.   | Controlled verification |
| DATASSET-006 | P1       | Garbage collection shall produce a dry-run report and auditable deletion result.                          | Controlled verification |
    ...
    - A versioned `dat_asset_manifest` recording identity, timestamps, ownership, state, and provenance.
