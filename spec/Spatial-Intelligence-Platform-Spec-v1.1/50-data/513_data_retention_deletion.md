---
spec_id: DAT-RET
title: "Retention, Legal Hold, Deletion, and Portability"
version: 1.1.0
...
normative: true
---
# Retention, Legal Hold, Deletion, and Portability

    ## 1. Purpose

    Defines lifecycle policies for originals, derivatives, models, logs, consent records, exports, backups, and customer departure.
    ...
    - Retention is assigned by data class, purpose, contract, consent, and jurisdiction.
- Deletion evaluates shared rights and derivative dependency rather than simply removing one database row.
- Cryptographic erasure is used where encrypted object deletion from all replicas is impractical within immediate time.
- Customer portability includes originals, canonical manifests, open derivatives, metadata, history, and integrity information.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| DATRET-001 | P0       | Every stored asset and record shall have retention class, policy source, hold status, and deletion eligibility.                      | Controlled verification |
| DATRET-002 | P0       | A deletion request shall produce impact analysis, authorization, execution plan, completion evidence, and unresolved exceptions.     | Controlled verification |
| DATRET-003 | P0       | Consent revocation shall stop future processing immediately and trigger policy-defined handling of prior derivatives.                | Controlled verification |
| DATRET-004 | P1       | Backups shall expire deleted data within a documented bounded period while preventing ordinary restoration from resurrecting access. | Automated test          |
| DATRET-005 | P1       | Tenant offboarding shall provide a verified export and deletion/retention report.                                                    | Controlled verification |
| DATRET-006 | P1       | Deletion shall propagate to derived search/vector indexes and cached viewer assets.                                                  | Controlled verification |
    ...
    - A versioned `dat_ret_manifest` recording identity, timestamps, ownership, state, and provenance.
