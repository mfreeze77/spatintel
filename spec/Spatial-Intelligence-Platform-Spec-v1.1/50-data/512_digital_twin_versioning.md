---
spec_id: DAT-TWIN
title: "Digital Twin Timeline and State Projection"
version: 1.1.0
...
normative: true
---
# Digital Twin Timeline and State Projection

    ## 1. Purpose

    Defines how capture events, accepted changes, system records, and memories form time-aware views of a place.
    ...
    - A digital twin is a projection over immutable history, not one mutable latest mesh.
- Valid time and transaction time are recorded for important entities and assertions.
- Users can compare selected dates/commits and inspect evidence for transitions.
- Predicted or proposed future state is visually and semantically separate from observed history.
...
| ----------- | -------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------- |
| DATTWIN-001 | P0       | The platform shall query scene/entity state as of a commit, valid date, or field visit.                          | Controlled verification |
| DATTWIN-002 | P0       | Changes shall reference causing evidence or workflow event when known.                                           | Automated test          |
| DATTWIN-003 | P0       | Late-arriving historical evidence shall be added without rewriting when it was received.                         | Controlled verification |
| DATTWIN-004 | P1       | A timeline view shall distinguish capture date, event date, document date, interview date, and publication date. | Automated test          |
| DATTWIN-005 | P1       | Future/proposed states shall not appear in current as-built views by default.                                    | Controlled verification |
| DATTWIN-006 | P1       | Retention/deletion shall preserve required audit chronology while honoring privacy policy.                       | Controlled verification |
    ...
    - A versioned `dat_twin_manifest` recording identity, timestamps, ownership, state, and provenance.
