---
spec_id: DAT-DB
title: "Transactional, Spatial, Search, and Graph Storage"
version: 1.1.0
...
normative: true
---
# Transactional, Spatial, Search, and Graph Storage

    ## 1. Purpose

    Defines reference storage using PostgreSQL/PostGIS, object storage, cache/queue, search/vector indexes, and optional graph projections.
    ...
    - PostgreSQL is the system of record for transactional metadata and durable workflow state; PostGIS stores searchable spatial envelopes, points, paths, and selected geometry.
- Large point clouds, meshes, splats, images, video, audio, and package assets live in content-addressed object storage.
- Search and vector indexes are projections and can be rebuilt.
- Graph queries may use relational recursive queries or a derived graph store without changing canonical ownership.
...
| --------- | -------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| DATDB-001 | P0       | Primary tables shall use tenant-aware keys, immutable creation fields, revision/version columns, and audited state transitions. | Controlled verification |
| DATDB-002 | P0       | Spatial indexes shall use declared SRID/frame semantics and shall not mix local frames as if they shared one CRS.               | Controlled verification |
| DATDB-003 | P0       | Object references shall include hash, size, media type, encryption/key scope, storage class, retention, and verification time.  | Automated test          |
| DATDB-004 | P1       | Database migrations shall be forward-tested, rollback/recovery-tested, and rehearsed against production-sized copies.           | Controlled verification |
| DATDB-005 | P1       | Derived indexes shall include source sequence/checkpoint so freshness can be measured.                                          | Controlled verification |
| DATDB-006 | P1       | Deletion and legal-hold logic shall traverse assets, derivatives, indexes, exports, and backups according to policy.            | Controlled verification |
    ...
    - A versioned `dat_db_manifest` recording identity, timestamps, ownership, state, and provenance.
