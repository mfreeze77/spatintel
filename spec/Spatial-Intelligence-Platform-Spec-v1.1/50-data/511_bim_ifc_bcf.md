---
spec_id: DAT-BIM
title: "BIM, IFC 4.3, BCF, and Design/As-Built Mapping"
version: 1.1.0
...
normative: true
---
# BIM, IFC 4.3, BCF, and Design/As-Built Mapping

    ## 1. Purpose

    Defines import, semantic mapping, spatial alignment, issue exchange, and the separation of design intent from observed condition.
    ...
    - IFC import creates a design branch and never overwrites field-observed entities.
- Entity mappings retain IFC GlobalId and source model revision.
- BCF issues can link viewpoints and SIP spatial anchors.
- IfcOpenShell may be used through an LGPL-compatible service boundary after legal review.
...
| ---------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| DATBIM-001 | P0       | IFC imports shall retain source file hash, schema/version, units, CRS, owner history, GlobalIds, classifications, properties, relationships, and geometry conversion report. | Automated test          |
| DATBIM-002 | P0       | Alignment between BIM and field scene shall be an explicit transform solution with residuals and controls.                                                                   | Controlled verification |
| DATBIM-003 | P0       | Field objects shall link to design objects through proposed/accepted mappings rather than ID replacement.                                                                    | Controlled verification |
| DATBIM-004 | P1       | BCF import/export shall preserve issue ID, status, author, comments, snapshots, viewpoints, selected components, and coordinate semantics.                                   | Controlled verification |
| DATBIM-005 | P1       | Unsupported IFC constructs shall be listed rather than silently flattened.                                                                                                   | Controlled verification |
| DATBIM-006 | P1       | As-built export shall identify which properties are verified, observed, inferred, or carried from design.                                                                    | Controlled verification |
    ...
    - A versioned `dat_bim_manifest` recording identity, timestamps, ownership, state, and provenance.
