---
spec_id: DAT-ONTO
title: "Ontology, Taxonomy, and Extensibility"
version: 1.1.0
...
normative: true
---
# Ontology, Taxonomy, and Extensibility

    ## 1. Purpose

    Defines shared base classes and vertical extensions for buildings, systems, objects, people, media, memories, and workflows.
    ...
    - The core ontology remains small: Place, PhysicalObject, Person/Agent, Event, Document/Media, System, Observation, Assertion, Task, and Collection.
- Construction maps to IFC and industry vocabularies through explicit mappings rather than copying all IFC classes into the core.
- LiveForever extends events, relationships, memories, media, and consent.
- User-defined types are namespaced and versioned.
...
| ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| DATONTO-001 | P0       | Every type shall define required properties, allowed relationships, authority rules, sensitivity defaults, and export mappings. | Controlled verification |
| DATONTO-002 | P0       | Vocabulary changes shall provide migration for stored type identifiers and search indexes.                                      | Controlled verification |
| DATONTO-003 | P0       | Aliases and multilingual labels shall not change stable type IDs.                                                               | Controlled verification |
| DATONTO-004 | P1       | The system shall support an unknown/unclassified state rather than forcing low-confidence classification.                       | Controlled verification |
| DATONTO-005 | P1       | Object detection labels shall map through a versioned model-label-to-ontology table and retain original labels.                 | Controlled verification |
| DATONTO-006 | P1       | Ontology constraints shall be validated at commit time with reviewable warnings or errors.                                      | Automated test          |
    ...
    - A versioned `dat_onto_manifest` recording identity, timestamps, ownership, state, and provenance.
