---
spec_id: DAT-ANCH
title: "Annotations, Spatial Anchors, and Measurements"
version: 1.1.0
...
normative: true
---
# Annotations, Spatial Anchors, and Measurements

    ## 1. Purpose

    Defines points, lines, areas, volumes, surface anchors, camera-view anchors, labels, notes, tasks, and measurement evidence.
    ...
    - Anchors reference named coordinate frames and stable scene/entity context.
- Measurements are records with method and uncertainty, not merely rendered labels.
- Annotations can be private, project, role-restricted, or published.
- Anchors include remapping strategy when geometry is superseded.
...
| ----------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| DATANCH-001 | P0       | A measurement shall include type, endpoints/geometry, value, units, source method, uncertainty, calibration/control, creator, time, evidence, and verification status. | Controlled verification |
| DATANCH-002 | P0       | The system shall distinguish a user click on a visual mesh from a snap to accepted metric geometry.                                                                    | Controlled verification |
| DATANCH-003 | P0       | Anchor remapping shall report residual/error and require review beyond threshold.                                                                                      | Controlled verification |
| DATANCH-004 | P1       | A task annotation shall include assignee, status, due date, related entity/assertion/evidence, and scene revision.                                                     | Controlled verification |
| DATANCH-005 | P1       | Spatial-volume annotations shall support privacy, issue, safety, and processing-exclusion policies.                                                                    | Controlled verification |
| DATANCH-006 | P1       | Exports shall retain measurement provenance even when target formats only support text labels.                                                                         | Controlled verification |
    ...
    - A versioned `dat_anch_manifest` recording identity, timestamps, ownership, state, and provenance.
...
## 15. Version 1.1 support maps and proxy interaction

A proxy can accelerate picking, but semantic anchors remain world/entity based and reprojectable. Measurement creation separates the initial visual/proxy click from the surface that supplies the saved value. When no eligible metric support exists, the result is visual reference only or requires field verification.

| ID | Priority | Requirement | Verification |
|---|---|---|---|
| DATANCH-007 | P0 | Proxy-local primitive identifiers shall not be the sole persistent anchor identity. | Remesh test |
| DATANCH-008 | P0 | Measurement save shall record both the initial interaction hit and the eligible metric/field evidence used for the value. | End-to-end test |
| DATANCH-009 | P0 | A proxy-only result shall remain unverified and shall not be exported as a verified dimension. | Policy/export test |
| DATANCH-010 | P1 | Support maps shall report reprojection residual, uncertainty, failed anchors, and review state. | Contract test |
| DATANCH-011 | P1 | Ambiguous overlapping semantic candidates shall be preserved and presented for resolution rather than silently selecting one. | UI test |
| DATANCH-012 | P1 | Anchors shall survive representation LOD and partition changes within the accepted support tolerance. | Streaming regression test |
