---
spec_id: CON-DOC
title: "Drawings, Specifications, RFIs, Submittals, and Spatial Linking"
version: 1.1.0
...
normative: true
---
# Drawings, Specifications, RFIs, Submittals, and Spatial Linking

    ## 1. Purpose

    Defines document ingestion, sheet/page regions, revisions, references, redlines, spatial anchors, and construction correspondence.
    ...
    - Original documents and revisions are immutable; current views are projections.
- A spatial link can target a sheet region, specification clause, submittal item, RFI question/response, or field condition.
- OCR/extraction is a derivative and never replaces the source page.
- Document status and contractual authority are imported or assigned by accountable users, not inferred from filename.
...
| ---------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CONDOC-001 | P0       | Documents shall retain source file hash, title, type, revision, issue date, issuer, status, pages, permissions, and supersession links.   | Automated test          |
| CONDOC-002 | P0       | Sheet anchors shall store page coordinate system, polygon/region, label, entity/space links, and evidence.                                | Controlled verification |
| CONDOC-003 | P0       | RFI records shall support question, context, responsible parties, due dates, responses, attachments, status, and affected scene/entities. | Controlled verification |
| CONDOC-004 | P1       | Submittal records shall support specification section, product/equipment mappings, review status, exceptions, and approved documents.     | Controlled verification |
| CONDOC-005 | P1       | The viewer shall open the exact referenced document revision and region from a spatial entity.                                            | Controlled verification |
| CONDOC-006 | P1       | AI extraction shall cite page/region and confidence and require review before authoritative mappings.                                     | Controlled verification |
    ...
    - A versioned `con_doc_manifest` recording identity, timestamps, ownership, state, and provenance.
