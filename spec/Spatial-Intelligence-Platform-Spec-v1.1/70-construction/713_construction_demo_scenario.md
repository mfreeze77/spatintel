---
spec_id: CON-DEMO
title: "Construction Demonstration Scenario"
version: 1.1.0
...
normative: true
---
# Construction Demonstration Scenario

    ## 1. Purpose

    Provides an end-to-end demonstration script centered on one building systems room and adjacent controlled openings.
    ...
    - The demo uses real workflow language and visible source/verification labels.
- The same capture becomes metric scene, visual scene, evidence graph, inventory, issue, and report.
- A second visit demonstrates Spatial Git and change detection.
- The demo deliberately includes one inaccessible area and one unverified measurement.
...
| ----------- | -------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CONDEMO-001 | P0       | The operator shall create a project, import a plan/drawing, capture the room, recover from a pause, and finalize locally.     | Controlled verification |
| CONDEMO-002 | P0       | The platform shall ingest, process preview/final, align LingBot and metric lanes, publish a reviewed scene, and show quality. | Controlled verification |
| CONDEMO-003 | P0       | The reviewer shall anchor a panel, devices, door hardware, equipment, label photos, drawing regions, and one RFI.             | Controlled verification |
| CONDEMO-004 | P1       | A measurement shall be shown first as scan estimate and later as field verified with history.                                 | Controlled verification |
| CONDEMO-005 | P1       | A second capture shall identify a changed object and produce a reviewable semantic diff.                                      | Controlled verification |
| CONDEMO-006 | P1       | The demo shall export a report and open-format project bundle while hiding restricted details from a broad-share viewer.      | Controlled verification |
    ...
    - A versioned `con_demo_manifest` recording identity, timestamps, ownership, state, and provenance.
...
The demo now includes a visual splat, calibrated metric surface, interaction proxy, collision/navigation product, proposed design object, semantic/evidence overlays, audience-redacted edition, proxy regeneration, and one intentionally blocked proxy-only verified measurement. Acceptance details are in [`714_hybrid_representation_construction.md`](714_hybrid_representation_construction.md) and [`../95-testing-delivery/961_splat_surface_benchmark_acceptance.md`](../95-testing-delivery/961_splat_surface_benchmark_acceptance.md).
