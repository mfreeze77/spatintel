---
spec_id: CON-TEST
title: "Construction Vertical Acceptance Tests"
version: 1.1.0
...
normative: true
---
# Construction Vertical Acceptance Tests

    ## 1. Purpose

    Defines field and system tests proving the construction product is useful, truthful, secure, and portable.
    ...
    - Acceptance uses a controlled building area with independent measurements and known inventory.
- Tests cover ordinary field workflow, not only prepared lab scans.
- Sensitive-system permissions are tested end-to-end.
- Open export and owner handoff are release gates.
...
| ----------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| CONTEST-001 | P0       | The pilot shall capture at least one mechanical/electrical room, corridor, controlled door, and fire-alarm device set using the same platform. | Controlled verification |
| CONTEST-002 | P0       | Independent ground truth shall measure selected dimensions, device locations, and inventory.                                                   | Controlled verification |
| CONTEST-003 | P0       | The pilot shall demonstrate an RFI/document link, issue lifecycle, return-visit change, and commissioning/test evidence.                       | Controlled verification |
| CONTEST-004 | P1       | Unauthorized roles shall be unable to discover restricted security-system entities through search, viewer, exports, counts, or URLs.           | Controlled verification |
| CONTEST-005 | P1       | A full project export shall open in the reference offline viewer and validated third-party format consumers.                                   | Automated test          |
| CONTEST-006 | P1       | Operators shall complete capture recovery after an intentional app interruption without source loss.                                           | Controlled verification |
    ...
    - A versioned `con_test_manifest` recording identity, timestamps, ownership, state, and provenance.
