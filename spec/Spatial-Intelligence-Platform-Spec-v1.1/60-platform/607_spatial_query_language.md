---
spec_id: PLT-SQL
title: "Spatial Query Language and Saved Views"
version: 1.1.0
...
normative: true
---
# Spatial Query Language and Saved Views

    ## 1. Purpose

    Defines a safe domain query model for locations, entities, evidence, time, source class, relationships, and workflow state.
    ...
    - The public query language compiles to authorized service queries and cannot expose raw SQL.
- Spatial predicates name frames or use place/room scopes.
- Queries can be saved, shared, scheduled, and used as report inputs with immutable versions.
- Natural-language interpretation produces a visible structured query before sensitive actions.
...
| ---------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| PLTSQL-001 | P0       | The language shall support `within`, `intersects`, `near`, `visible_from`, `on_level`, `changed_between`, temporal intervals, graph traversal, source/authority, and evidence filters. | Controlled verification |
| PLTSQL-002 | P0       | Query execution shall enforce complexity, result, time, and spatial-volume limits.                                                                                                     | Controlled verification |
| PLTSQL-003 | P0       | Saved queries shall record schema version, author, permissions, parameters, and expected result contract.                                                                              | Automated test          |
| PLTSQL-004 | P1       | Natural-language translation shall not broaden scope beyond the user's authorization or stated intent.                                                                                 | Controlled verification |
| PLTSQL-005 | P1       | Reports shall record the exact query and scene commit used.                                                                                                                            | Controlled verification |
| PLTSQL-006 | P1       | Query explanations shall show filters and ranking factors in user-understandable form.                                                                                                 | Controlled verification |
    ...
    - A versioned `plt_sql_manifest` recording identity, timestamps, ownership, state, and provenance.
