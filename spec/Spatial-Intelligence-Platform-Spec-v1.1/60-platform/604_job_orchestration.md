---
spec_id: PLT-JOB
title: "Pipeline Job Orchestration and Review Gates"
version: 1.1.0
...
normative: true
---
# Pipeline Job Orchestration and Review Gates

    ## 1. Purpose

    Defines DAGs/state machines for validation, decode, reconstruction, optimization, fusion, semantics, publication, export, and manual review.
    ...
    - Workflows are durable and activities are idempotent.
- Preview and final runs are separate lineage branches.
- Manual review is a first-class wait state with deadline, escalation, and evidence.
- A failed optional stage can produce a partial preview only when policy explicitly permits it.
...
| ---------- | -------- | ----------------------------------------------------------------------------------------------------------------- | ----------------------- |
| PLTJOB-001 | P0       | A workflow shall persist state, attempts, checkpoints, input/output manifests, costs, warnings, and cancellation. | Automated test          |
| PLTJOB-002 | P0       | Activity retries shall not duplicate published assets or charges.                                                 | Controlled verification |
| PLTJOB-003 | P0       | Quality and license gates shall execute before publication.                                                       | Controlled verification |
| PLTJOB-004 | P1       | Manual-review tasks shall include scope, evidence, requested decision, possible outcomes, and impact.             | Controlled verification |
| PLTJOB-005 | P1       | Workflow upgrades shall preserve running jobs or migrate them explicitly.                                         | Controlled verification |
| PLTJOB-006 | P1       | Operators shall be able to pause queues or disable a model/profile without editing customer data.                 | Controlled verification |
    ...
    - A versioned `plt_job_manifest` recording identity, timestamps, ownership, state, and provenance.
...
## Version 1.1 durable workflows

Hybrid-representation workflows separate admission, execution, quarantine, validation, review, publication, and supersession. Retry creates a new immutable attempt, cancellation quarantines partial output, and publication has exactly-once effect through the scene commit transaction. Provider expiry or policy drift is rechecked at publication.
