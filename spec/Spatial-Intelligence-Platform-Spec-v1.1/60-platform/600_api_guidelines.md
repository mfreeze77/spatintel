---
spec_id: PLT-API
title: "API Design, Versioning, and Error Semantics"
version: 1.1.0
...
normative: true
---
# API Design, Versioning, and Error Semantics

    ## 1. Purpose

    Defines resource identity, versioning, idempotency, pagination, filtering, concurrency, authorization, errors, and compatibility.
    ...
    - Public APIs use stable resource IDs and explicit schema/media versions.
- Create/mutate operations support idempotency keys and optimistic concurrency.
- Large work returns operations/jobs rather than holding HTTP connections.
- Errors are machine-readable, safe for users, correlated for operators, and never leak unauthorized existence.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| PLTAPI-001 | P0       | Every endpoint shall document authentication, authorization, request/response schema, idempotency, side effects, events, errors, limits, and examples. | Automated test          |
| PLTAPI-002 | P0       | Mutation requests shall include an idempotency key or be demonstrably naturally idempotent.                                                            | Automated test          |
| PLTAPI-003 | P0       | Conditional updates shall use version/ETag semantics and reject lost updates.                                                                          | Automated test          |
| PLTAPI-004 | P1       | Pagination shall use opaque stable cursors and deterministic ordering.                                                                                 | Controlled verification |
| PLTAPI-005 | P1       | Error responses shall include stable code, message, trace/correlation ID, retryability, and field details when safe.                                   | Controlled verification |
| PLTAPI-006 | P1       | Breaking API changes shall use a new major version and migration period.                                                                               | Automated test          |
    ...
    - A versioned `plt_api_manifest` recording identity, timestamps, ownership, state, and provenance.
