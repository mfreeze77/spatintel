---
spec_id: PLT-GRPC
title: "Internal Worker and Streaming Contracts"
version: 1.1.0
...
normative: true
---
# Internal Worker and Streaming Contracts

    ## 1. Purpose

    Defines authenticated control-plane-to-worker execution, heartbeats, progress, checkpoints, artifacts, cancellation, and bounded streaming.
    ...
    - Workers receive signed job leases and content-addressed manifests, not broad database access.
- gRPC or equivalent typed RPC is used for low-latency internal control; durable job truth remains in orchestration state.
- Artifact transfer occurs through object storage/local mounts, not unbounded RPC payloads.
- Worker protocol supports capability discovery and version negotiation.
...
| ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| PLTGRPC-001 | P0       | A job lease shall include job/run ID, adapter/profile, input manifest, output staging scope, resource limits, deadline, cancellation token, and workload identity. | Controlled verification |
| PLTGRPC-002 | P0       | Workers shall report startup, download, decode, stage progress, warnings, quality, checkpoints, resource use, and terminal result.                                 | Controlled verification |
| PLTGRPC-003 | P0       | Heartbeat expiry shall release or quarantine the lease according to checkpoint safety.                                                                             | Controlled verification |
| PLTGRPC-004 | P1       | The control plane shall verify output hashes and manifest before accepting completion.                                                                             | Automated test          |
| PLTGRPC-005 | P1       | Protocol mismatches shall fail before expensive work and never fall back to untyped execution.                                                                     | Controlled verification |
| PLTGRPC-006 | P1       | Workers shall support graceful cancellation at declared safe points.                                                                                               | Controlled verification |
    ...
    - A versioned `plt_grpc_manifest` recording identity, timestamps, ownership, state, and provenance.
...
## Version 1.1 worker contract

Splat-surface, cleanup, collision/navigation, and quality workers implement the provider SDK and use scoped immutable input handles plus write-only quarantine output handles. Workers cannot publish scene state. Exact request/result envelopes, resource bounds, cancellation, and receipt behavior are governed by [`614_splat_surface_api_and_events.md`](614_splat_surface_api_and_events.md).
