---
spec_id: PLT-DESK
title: "Desktop Expert Review Application"
version: 1.1.0
...
normative: true
---
# Desktop Expert Review Application

    ## 1. Purpose

    Defines optional high-performance local review for raw captures, alignment, point clouds, quality, correspondence, and offline projects.
    ...
    - The MVP may begin with a browser plus local worker, but the contracts permit a native macOS/Windows reviewer.
- Expert tools expose raw frames, depth, poses, residuals, graphs, masks, and source contributions.
- Local edits create proposals/commits through the same domain rules as web edits.
- Offline project bundles are signed and merge through Spatial Git rules.
...
| ----------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| PLTDESK-001 | P0       | The reviewer shall inspect synchronized frame/depth/trajectory/geometry and allow correspondence creation with undo and provenance. | Controlled verification |
| PLTDESK-002 | P0       | The reviewer shall visualize factor residuals, loop constraints, source weights, and region uncertainty.                            | Controlled verification |
| PLTDESK-003 | P0       | Offline changes shall preserve local identity, author, base commit, and conflict information.                                       | Controlled verification |
| PLTDESK-004 | P1       | The application shall not require cloud login for authorized local-only projects.                                                   | Controlled verification |
| PLTDESK-005 | P1       | GPU crashes shall not corrupt local project bundles.                                                                                | Controlled verification |
| PLTDESK-006 | P1       | Exports and publication shall still pass policy and license gates.                                                                  | Controlled verification |
    ...
    - A versioned `plt_desk_manifest` recording identity, timestamps, ownership, state, and provenance.
