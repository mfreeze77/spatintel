...
# Adapter and Plugin SDK
    ...
    Defines safe extension points for capture importers, algorithms, semantic extractors, exporters, reports, and enterprise connectors.
    ...
    - Plugins operate through declared capabilities and sandboxed interfaces.
- A plugin cannot bypass policy, write canonical tables, or fetch undeclared network resources.
- Plugin packages include license/SBOM, signatures, permissions, schemas, tests, and compatibility range.
- Customer-private plugins can run in isolated tenant workers.
...
| ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| PLTSDK-001 | P0       | The SDK shall define contracts for source adapters, pipeline stages, quality metrics, entity extractors, exporters, viewer layers, and report generators. | Controlled verification |
| PLTSDK-002 | P0       | Plugin installation shall require signature verification, dependency scan, requested capabilities, and administrator approval.                            | Controlled verification |
| PLTSDK-003 | P0       | Runtime shall enforce CPU/GPU/memory/time/network/filesystem limits.                                                                                      | Controlled verification |
| PLTSDK-004 | P1       | Plugin outputs shall pass schema, provenance, quality, and authorization validation before publication.                                                   | Automated test          |
| PLTSDK-005 | P1       | A plugin failure shall not corrupt the parent workflow or leak another tenant's data.                                                                     | Controlled verification |
| PLTSDK-006 | P1       | Compatibility tests shall run against supported contract versions.                                                                                        | Controlled verification |
    ...
    - A versioned `plt_sdk_manifest` recording identity, timestamps, ownership, state, and provenance.
...
## Version 1.1 spatial-provider SDK

The plugin boundary now includes a separately governed spatial-provider SDK. Provider manifests declare exact artifacts, licenses, data classes, network behavior, hardware, capabilities, formats, roles, benchmarks, and expiration. Adapters produce quarantined candidates and cannot publish or increase authority.
