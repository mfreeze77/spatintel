---
spec_id: PLT-MODEL
title: "Model Registry and Deployment Control"
version: 1.1.0
...
normative: true
---
# Model Registry and Deployment Control

    ## 1. Purpose

    Defines model/checkpoint identity, approval, artifacts, evaluation, routing, rollback, monitoring, and revocation.
    ...
    - The registry references immutable checkpoint hashes and approval evidence.
- A model can be approved per purpose, data class, deployment profile, region, customer class, and date.
- Routing uses named profiles rather than untracked latest aliases.
- Revocation immediately blocks new jobs and identifies affected derivatives.
...
| ------------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| PLTMODEL-001 | P0       | A model version shall include architecture/source, checkpoint hash, tokenizer/preprocessor, license, data terms, container, hardware compatibility, benchmark results, limitations, and approvers. | Automated test          |
| PLTMODEL-002 | P0       | Production workers shall verify registry state and checkpoint hash at job start.                                                                                                                   | Automated test          |
| PLTMODEL-003 | P0       | Shadow/canary runs shall compare quality, cost, latency, safety, and failure modes before promotion.                                                                                               | Controlled verification |
| PLTMODEL-004 | P1       | Rollback shall be one control-plane operation and preserve runs generated by the revoked model for audit/rebuild analysis.                                                                         | Controlled verification |
| PLTMODEL-005 | P1       | Monitoring shall segment quality and incident metrics by model version.                                                                                                                            | Controlled verification |
| PLTMODEL-006 | P1       | Auxiliary models such as sky masks, segmentation, OCR, embeddings, and voice models shall follow the same registry rules.                                                                          | Controlled verification |
    ...
    - A versioned `plt_model_manifest` recording identity, timestamps, ownership, state, and provenance.
