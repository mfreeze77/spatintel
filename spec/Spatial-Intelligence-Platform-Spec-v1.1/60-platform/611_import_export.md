---
spec_id: PLT-IO
title: "Import, Export, Packaging, and Migration"
version: 1.1.0
...
normative: true
---
# Import, Export, Packaging, and Migration

    ## 1. Purpose

    Defines user-controlled movement of projects, scene history, assets, BIM, reports, and legacy systems.
    ...
    - Imports are staged, validated, mapped, previewed, and committed; they do not mutate live projects directly.
- Exports are immutable jobs with scope, policy, redaction, format, and integrity manifests.
- A full portability bundle can recreate a read-only project without SIP cloud.
- Migration tools preserve source identifiers and mapping reports.
...
| --------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| PLTIO-001 | P0       | An import shall report accepted, transformed, skipped, duplicate, conflicting, unsupported, and quarantined items.                                           | Controlled verification |
| PLTIO-002 | P0       | An export shall include source commit, query/scope, permissions/consent decision, redactions, transformations, output hashes, and limitations.               | Automated test          |
| PLTIO-003 | P0       | Full-project export shall include originals when authorized, canonical manifests, scene graph, history, evidence, open-format geometry/media, and checksums. | Controlled verification |
| PLTIO-004 | P1       | The system shall support dry-run migration and deterministic re-execution.                                                                                   | Controlled verification |
| PLTIO-005 | P1       | Password-protected or encrypted input shall never be weakened or logged without explicit authorized handling.                                                | Automated test          |
| PLTIO-006 | P1       | Portability verification shall run on a clean offline viewer/reference importer.                                                                             | Controlled verification |
    ...
    - A versioned `plt_io_manifest` recording identity, timestamps, ownership, state, and provenance.
...
## 15. Version 1.1 hybrid representation portability

Hybrid exports include open representation assets where permitted plus sidecars for role, authority, coordinate frames, transforms, derivation, support maps, intended-use validation, limitations, policies, redaction/generation labels, and checksums. A GLB or splat file alone is not represented as a complete evidentiary export.

| ID | Priority | Requirement | Verification |
|---|---|---|---|
| PLTIO-007 | P0 | Hybrid export shall preserve role, authority, transform, derivation, use suitability, limitations, support maps, and policy labels. | Independent import test |
| PLTIO-008 | P0 | External/manual provider export shall be minimized, purpose-limited, hashed, expiring, and receipted. | Security workflow test |
| PLTIO-009 | P0 | Import shall quarantine unknown providers, roles, extensions, transforms, and authority claims. | Adversarial import test |
| PLTIO-010 | P1 | Full-project export shall include visual, metric, interaction, design, evidence, and non-splat fallback assets when authorized. | Portability test |
| PLTIO-011 | P1 | Format conversion shall retain irreversible-loss declarations and source relationships. | Round-trip test |
| PLTIO-012 | P1 | A clean reference importer shall reconstruct the hybrid layer registry without vendor-specific private knowledge. | Independent verification |

## Version 1.1 hybrid export

Exports preserve representation roles, authority, coordinate frames, source/derivation lineage, validation, limitations, support maps, partitions, LOD families, consent/audience policy, and checksums. At least one broadly documented fallback is included where permitted. Conversion is never described as lossless.
