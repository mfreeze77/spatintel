---
spec_id: OPS-HYB
title: "Cloud, Hybrid, Edge, and Local Deployment"
version: 1.1.0
...
normative: true
---
# Cloud, Hybrid, Edge, and Local Deployment

    ## 1. Purpose

    Defines reference patterns, data-flow policy, connectivity, updates, device enrollment, and operational tradeoffs across deployment modes.
    ...
    - Local-only is viable for privacy-sensitive LiveForever and secure construction projects.
- Hybrid supports local raw storage with cloud metadata or approved derivatives.
- Cloud supports elastic multi-tenant processing but never becomes the sole export format.
- Deployment mode is project policy and can restrict model providers and regions.
...
| ---------- | -------- | -------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| OPSHYB-001 | P0       | Every deployment shall expose the same canonical package, scene, evidence, and export contracts.                                 | Controlled verification |
| OPSHYB-002 | P0       | Hybrid transfer policy shall specify which asset classes may leave the site and at what processing stage.                        | Controlled verification |
| OPSHYB-003 | P0       | Edge nodes shall enroll with unique identity, signed software, disk encryption, health reporting, and revocation.                | Automated test          |
| OPSHYB-004 | P1       | Offline updates shall be signed and rollback-capable.                                                                            | Controlled verification |
| OPSHYB-005 | P1       | A project migration between deployment modes shall preserve IDs, history, permissions, consent, hashes, and model/run manifests. | Automated test          |
| OPSHYB-006 | P1       | Cloud dependence shall be visible to administrators before enabling a feature.                                                   | Controlled verification |
    ...
    - A versioned `ops_hyb_manifest` recording identity, timestamps, ownership, state, and provenance.
