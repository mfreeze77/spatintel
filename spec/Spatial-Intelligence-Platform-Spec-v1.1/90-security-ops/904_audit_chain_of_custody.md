---
spec_id: OPS-AUDIT
title: "Audit, Integrity, and Chain of Custody"
version: 1.1.0
...
normative: true
---
# Audit, Integrity, and Chain of Custody

    ## 1. Purpose

    Defines tamper-evident records for capture finalization, evidence handling, processing, review, access, export, and deletion.
    ...
    - Audit events are append-only and separate from mutable operational logs.
- High-value manifests and releases can be signed and anchored through hash chains.
- Chain-of-custody supports trust and investigation but does not automatically confer legal admissibility.
- Clock uncertainty and offline actions are preserved.
...
| ------------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| OPSAUDIT-001 | P0       | Audit events shall include actor/workload, action, resource, tenant/project, time, source IP/device context when appropriate, purpose, outcome, trace, and before/after references. | Automated test          |
| OPSAUDIT-002 | P0       | Capture finalization shall record package root hash, device/app, signer, and verification result.                                                                                   | Automated test          |
| OPSAUDIT-003 | P0       | Evidence access, model execution, authority changes, consent changes, exports, and deletion shall be audited.                                                                       | Controlled verification |
| OPSAUDIT-004 | P1       | Audit storage shall prevent ordinary application roles from altering or deleting events.                                                                                            | Automated test          |
| OPSAUDIT-005 | P1       | Verification tools shall detect gaps, broken chains, invalid signatures, and missing referenced manifests.                                                                          | Controlled verification |
| OPSAUDIT-006 | P1       | Legal/evidentiary claims shall require separate counsel and jurisdictional procedure.                                                                                               | Controlled verification |
    ...
    - A versioned `ops_audit_manifest` recording identity, timestamps, ownership, state, and provenance.
