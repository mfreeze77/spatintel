---
spec_id: OPS-PRIV
title: "Privacy Engineering and Compliance Controls"
version: 1.1.0
...
normative: true
---
# Privacy Engineering and Compliance Controls

    ## 1. Purpose

    Defines data inventory, purpose limitation, minimization, consent, subject rights, DPIA-style review, residency, vendors, and breach readiness.
    ...
    - Privacy is implemented through data classes and policy controls rather than a generic legal notice.
- Construction and LiveForever have different default sensitivities but share the same enforcement engine.
- Biometric/voice/likeness processing receives elevated review and opt-in consent.
- Jurisdiction-specific obligations are configuration and legal-review inputs, not hard-coded legal conclusions in the spec.
...
| ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| OPSPRIV-001 | P0       | The platform shall maintain a data inventory linking category, purpose, legal/consent basis, processors, residency, retention, security, and rights workflow. | Controlled verification |
| OPSPRIV-002 | P0       | New data purposes or providers shall require privacy impact review before processing.                                                                         | Controlled verification |
| OPSPRIV-003 | P0       | Subject-access export shall explain source records, derivatives, sharing, consent, and automated processing in understandable form.                           | Controlled verification |
| OPSPRIV-004 | P1       | Correction/restriction/deletion requests shall be tracked and verified across canonical and derived stores.                                                   | Controlled verification |
| OPSPRIV-005 | P1       | Vendor agreements and settings shall prohibit training/use beyond approved purpose where required.                                                            | Controlled verification |
| OPSPRIV-006 | P1       | Incident workflows shall identify affected people/projects and applicable notification decision without exposing additional data.                             | Controlled verification |
    ...
    - A versioned `ops_priv_manifest` recording identity, timestamps, ownership, state, and provenance.
...
## Version 1.1 privacy additions

Spatial conversion can expose complete homes, family photographs, bystanders, security systems, network rooms, and critical infrastructure. Provider admission must enforce data class, consent, purpose, region, retention, deletion, subprocessors, and egress before bytes leave the approved boundary. Presentation redaction is not source deletion.
