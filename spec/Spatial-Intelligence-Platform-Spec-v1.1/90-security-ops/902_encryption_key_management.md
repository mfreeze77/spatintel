...
# Encryption and Key Management
    ...
    Defines encryption in transit/at rest, envelope keys, local-only keys, rotations, revocation, recovery, and cryptographic erasure.
    ...
    - Assets use envelope encryption with separable tenant/project/person key scopes as policy requires.
- Content hash identifies plaintext logical content while storage records protect ciphertext identity and integrity.
- Local-only deployments can keep root keys off cloud systems.
- Key rotation does not require rewriting canonical provenance.
...
| ---------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| OPSKEY-001 | P0       | All network connections shall use current approved TLS with certificate validation; local transfer uses authenticated encrypted channels. | Automated test          |
| OPSKEY-002 | P0       | Data keys shall be generated securely, wrapped by managed or local key-encryption keys, and scoped by policy.                             | Automated test          |
| OPSKEY-003 | P0       | Key access shall be logged with actor/workload, purpose, resource scope, and outcome.                                                     | Controlled verification |
| OPSKEY-004 | P1       | Rotation shall support rewrapping and emergency revocation with bounded impact.                                                           | Controlled verification |
| OPSKEY-005 | P1       | Recovery mechanisms shall avoid one-person permanent loss while respecting local-only and family succession policy.                       | Controlled verification |
| OPSKEY-006 | P1       | Cryptographic erasure shall include key deletion evidence and residual replica/backup timelines.                                          | Controlled verification |
    ...
    - A versioned `ops_key_manifest` recording identity, timestamps, ownership, state, and provenance.
