---
spec_id: OPS-PERF
title: "Performance and Scalability Budgets"
version: 1.1.0
...
# Performance and Scalability Budgets
    ...
    Defines measurable latency, throughput, viewer, mobile, upload, processing, query, and recovery targets by operating profile.
    ...
    - Targets are percentile-based and tied to hardware/input classes.
- Preview responsiveness is distinct from final processing time.
- Quality cannot be silently reduced merely to meet latency.
- The user sees progress and actionable degradation rather than indefinite spinners.
...
| ----------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| OPSPERF-001 | P0       | The iOS app shall maintain capture responsiveness and bounded frame-drop behavior on supported devices under declared thermal conditions. | Controlled verification |
| OPSPERF-002 | P0       | Upload shall saturate available policy-permitted bandwidth while preserving foreground responsiveness and resumability.                   | Controlled verification |
| OPSPERF-003 | P0       | API and search SLOs shall be defined by endpoint class and result size.                                                                   | Automated test          |
| OPSPERF-004 | P1       | The viewer shall define frame-time, first-meaningful-scene, memory, and network budgets by device tier and scene size.                    | Controlled verification |
| OPSPERF-005 | P1       | Pipeline profiles shall define expected runtime and peak VRAM/RAM per input class.                                                        | Controlled verification |
| OPSPERF-006 | P1       | Load tests shall include noisy-neighbor and tenant-quota behavior.                                                                        | Controlled verification |
    ...
    - A versioned `ops_perf_manifest` recording identity, timestamps, ownership, state, and provenance.
