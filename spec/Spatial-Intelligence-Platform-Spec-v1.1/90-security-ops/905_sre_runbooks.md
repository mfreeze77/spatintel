---
spec_id: OPS-SRE
title: "SRE Runbooks and Incident Operations"
version: 1.1.0
...
normative: true
---
# SRE Runbooks and Incident Operations

    ## 1. Purpose

    Defines alerts, triage, containment, remediation, recovery, communication, evidence preservation, and post-incident improvement.
    ...
    - Runbooks are scenario-based and executable under pressure.
- Security/privacy, data integrity, model quality, capacity, and provider outages have distinct incident classes.
- Stopping unsafe processing takes priority over completing queued jobs.
- Customer communication states known facts, impact, workarounds, and uncertainty.
...
| ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| OPSSRE-001 | P0       | Runbooks shall cover database outage, object corruption, queue backlog, GPU failure, model regression, cross-tenant suspicion, leaked share link, consent failure, bad redaction, deletion error, and backup restore. | Controlled verification |
| OPSSRE-002 | P0       | Each runbook shall include trigger, severity, roles, commands/controls, decision points, evidence, validation, rollback, and communications.                                                                          | Controlled verification |
| OPSSRE-003 | P0       | Incident actions shall be audited and preserve forensic evidence without copying sensitive data unnecessarily.                                                                                                        | Controlled verification |
| OPSSRE-004 | P1       | After-action reviews shall create tracked corrective requirements and tests.                                                                                                                                          | Controlled verification |
| OPSSRE-005 | P1       | Runbooks shall be exercised through game days.                                                                                                                                                                        | Controlled verification |
| OPSSRE-006 | P1       | On-call access shall use just-in-time privileges and recorded sessions where appropriate.                                                                                                                             | Controlled verification |
    ...
    - A versioned `ops_sre_manifest` recording identity, timestamps, ownership, state, and provenance.
