---
spec_id: OPS-CICD
title: "CI/CD, Software Supply Chain, and Release Engineering"
version: 1.1.0
...
normative: true
---
# CI/CD, Software Supply Chain, and Release Engineering

    ## 1. Purpose

    Defines source control, reviews, tests, signed builds, SBOMs, dependency/model locking, environments, migrations, rollout, and rollback.
    ...
    - Every deployable artifact is built from reviewed source in CI and signed/attested.
- Dependencies and model checkpoints are pinned and mirrored under approval.
- Production releases use staged rollout and rapid rollback.
- Database/schema migrations are independently rehearsed.
...
| ----------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| OPSCICD-001 | P0       | CI shall run formatting, type checks, unit/integration tests, schema compatibility, license/SBOM, secret scan, vulnerability scan, and artifact signing. | Automated test          |
| OPSCICD-002 | P0       | Mobile, web, service, worker, infrastructure, and model manifests shall share a release record.                                                          | Controlled verification |
| OPSCICD-003 | P0       | Production deployment shall verify signatures and environment policy.                                                                                    | Controlled verification |
| OPSCICD-004 | P1       | Release promotion shall require benchmark and acceptance evidence from `955_release_gates.md`.                                                           | Controlled verification |
| OPSCICD-005 | P1       | Emergency patches shall still produce traceable artifacts and retrospective review.                                                                      | Controlled verification |
| OPSCICD-006 | P1       | Rollback shall include service, worker routing/model, configuration, and database compatibility plans.                                                   | Controlled verification |
    ...
    - A versioned `ops_cicd_manifest` recording identity, timestamps, ownership, state, and provenance.
