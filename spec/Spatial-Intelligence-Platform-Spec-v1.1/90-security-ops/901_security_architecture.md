---
spec_id: OPS-SEC
title: "Security Architecture and Zero-Trust Controls"
version: 1.1.0
...
# Security Architecture and Zero-Trust Controls
    ...
    Defines authentication, authorization, workload identity, network segmentation, secrets, hardening, isolation, secure defaults, and incident evidence.
    ...
    - Every human and workload request is authenticated and authorized; internal network location alone grants no trust.
- Raw asset access uses narrowly scoped short-lived grants.
- GPU workers are disposable, isolated, and cannot reach unrelated tenant data.
- Production administration is separated from normal application accounts.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| OPSSEC-001 | P0       | Human privileged access shall require phishing-resistant MFA where available and just-in-time elevation.                 | Controlled verification |
| OPSSEC-002 | P0       | Service-to-service authentication shall use short-lived workload identities with audience and scope.                     | Controlled verification |
| OPSSEC-003 | P0       | Network policies shall restrict worker egress and prevent runtime model downloads in production.                         | Automated test          |
| OPSSEC-004 | P1       | Secrets shall live in an approved secret manager and never in repositories, images, logs, manifests, or mobile packages. | Controlled verification |
| OPSSEC-005 | P1       | Containers and mobile builds shall be hardened, signed, scanned, and patched under defined SLAs.                         | Controlled verification |
| OPSSEC-006 | P1       | Sensitive-access and administrative events shall be immutable and monitored.                                             | Automated test          |
    ...
    - A versioned `ops_sec_manifest` recording identity, timestamps, ownership, state, and provenance.
