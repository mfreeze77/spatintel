---
spec_id: APP-FAIL
title: "Failure Mode and Recovery Catalog"
version: 1.1.0
...
normative: true
---
# Failure Mode and Recovery Catalog

    ## 1. Purpose

    Provides a cross-system catalog of capture, data, reconstruction, model, viewer, privacy, security, and operational failures.

    This document is normative for the appendix layer. It defines behavior that must remain stable even when implementation libraries, models, cloud providers, or user interfaces change. The design favors explicit evidence and reversible decisions over hidden automation.

    ## 2. Outcomes

    - Failures have stable codes, detection signals, user effect, containment, recovery, and publication impact.
- Low confidence and out-of-envelope are explicit outcomes, not generic success.
- Data integrity failures take precedence over availability.
- Recovery never upgrades authority or removes source labels.

    ## 3. Governing principles

    - Examples are illustrative unless explicitly marked normative.
- External facts include source, access date, and a review status.
- Schemas and API snippets are kept consistent with the normative data model through validation tooling.
- Known unknowns remain explicit instead of being hidden behind optimistic assumptions.

    ## 4. Normative requirements

    | ID          | Priority | Requirement                                                                                                                                                                                                                                                                                                                                    | Verification            |
| ----------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| APPFAIL-001 | P0       | The catalog shall cover mobile interruption, tracking loss, corrupt package, hash mismatch, clock error, bad calibration, pose collapse, misalignment, false loop, depth-scale error, dynamic contamination, mesh defect, model/license block, queue/provider outage, index staleness, permission leak, redaction failure, and deletion error. | Automated test          |
| APPFAIL-002 | P0       | Each failure shall have severity, retryability, safe checkpoint, operator action, customer message, audit requirement, and test reference.                                                                                                                                                                                                     | Controlled verification |
| APPFAIL-003 | P0       | Fatal failures shall preserve diagnostic manifests without exposing sensitive raw data.                                                                                                                                                                                                                                                        | Controlled verification |
| APPFAIL-004 | P1       | A failure classified as unsafe shall prevent publication even when partial outputs exist.                                                                                                                                                                                                                                                      | Automated test          |
| APPFAIL-005 | P1       | Recovery shall create new run/commit lineage rather than rewriting the failed record.                                                                                                                                                                                                                                                          | Controlled verification |
| APPFAIL-006 | P1       | Failure-code compatibility shall be maintained for API clients and support analytics.                                                                                                                                                                                                                                                          | Automated test          |
    ...
    - A versioned `app_fail_manifest` recording identity, timestamps, ownership, state, and provenance.
...
5. Commit the new revision atomically, preserving the prior authoritative state.
    ...
    Examples contain no production secrets, personal data, customer identifiers, or sensitive facility details.
...
- Partial writes are rolled back or marked incomplete; they are never returned as successful revisions.
    ...
    Required operational measures include: broken link count, schema validation failures, stale source age, unresolved appendix TODOs. Metrics are segmented by tenant, project, capture source, hardware profile, algorithm/model version, and software release when cardinality is safe. Traces carry a correlation identifier from API request through events, worker jobs, storage operations, and publication.
...
## Version 1.1 hybrid-representation failure additions

| Failure | Safe behavior | Recovery |
|---|---|---|
| Surface provider unavailable or revoked | keep visual/metric/prior proxy available; block new external transfer | use approved local provider or retry after reviewed approval |
| Output transform/scale invalid | quarantine and prohibit scene publication | correct adapter/receipt and rerun validation |
| Proxy false bridge or missing opening | disable affected collision/navigation profile | regenerate with constraint, edit journal, or local fallback |
| Anchor remap exceeds threshold | retain historical support and mark unresolved | human review/re-anchor without rewriting source memory/evidence |
| Sensitive manual export attempted | deny before bytes leave approved boundary | use minimized synthetic fixture or approved local execution |
| Malicious/oversized geometry payload | isolate parser and reject/quarantine | patch parser/provider and rerun security suite |
| Proxy accidentally used for verification | block save/export and emit policy incident | resolve against metric/field evidence; review affected records |
| Redaction not propagated to derivative | withdraw derivative, purge caches, block publication | regenerate entire dependent derivative graph and verify |
| Immersive navigation unsafe | disable free movement, retain guided/2D safe mode | regenerate collision/nav and repeat safety acceptance |
| Splat renderer unsupported | preserve semantic/evidence experience | load preservation mesh, source panoramas, or bounded fallback |

| ID | Priority | Requirement | Verification |
|---|---|---|---|
| APPFAIL-007 | P0 | The failure catalog shall cover provider denial/revocation, malformed outputs, transform error, false geometry, anchor failure, redaction propagation, authority misuse, and immersive safety. | Failure-catalog audit |
| APPFAIL-008 | P0 | Hybrid failure shall preserve source, metric, visual, evidence, and prior accepted assets without partial publication. | Fault-injection test |
| APPFAIL-009 | P1 | Picking, collision, navigation, occlusion, audio, and visual rendering shall fail independently with explicit fallbacks. | Component failure test |
| APPFAIL-010 | P1 | Recovery shall create new operation/revision lineage and shall not erase failed provider evidence. | Recovery audit |
