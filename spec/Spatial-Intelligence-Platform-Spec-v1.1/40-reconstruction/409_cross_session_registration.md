---
spec_id: REC-XREG
title: "Cross-Session Relocalization and Registration"
version: 1.1.0
...
normative: true
---
# Cross-Session Relocalization and Registration

    ## 1. Purpose

    Defines how return visits, different devices, historical captures, and imported models align to a persistent place.
    ...
    - Registration uses hierarchical place retrieval, geometric verification, controls, and semantic landmarks.
- Each session retains its local coordinate frame even after global alignment.
- Significant building changes can create multiple valid local explanations rather than forcing a false rigid fit.
- Historical media may be approximately anchored with explicit uncertainty.
...
| ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| RECXREG-001 | P0       | The system shall generate candidate place matches using approved descriptors or manual selection and verify them geometrically. | Controlled verification |
| RECXREG-002 | P0       | Cross-session transforms shall report overlap, inliers, residuals, scale, uncertainty, and changed-region masks.                | Controlled verification |
| RECXREG-003 | P0       | The optimizer shall avoid using known changed objects as rigid constraints.                                                     | Controlled verification |
| RECXREG-004 | P1       | Manual correspondence tools shall support points, planes, lines, semantic objects, and image-to-scene anchors.                  | Controlled verification |
| RECXREG-005 | P1       | Registration failures shall leave sessions independently viewable and reviewable.                                               | Controlled verification |
| RECXREG-006 | P1       | A global-place revision shall preserve every contributing session and transform solution.                                       | Controlled verification |
    ...
    - A versioned `rec_xreg_manifest` recording identity, timestamps, ownership, state, and provenance.
