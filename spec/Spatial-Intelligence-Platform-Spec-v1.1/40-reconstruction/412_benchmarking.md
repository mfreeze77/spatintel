---
spec_id: REC-BENCH
title: "Geometry and Reconstruction Benchmark Program"
version: 1.1.0
...
normative: true
---
# Geometry and Reconstruction Benchmark Program

    ## 1. Purpose

    Defines representative datasets, ground truth, metrics, reproducibility, hardware profiles, and release comparison.
    ...
    - Benchmarks combine public datasets, synthetic scenes, controlled physical rooms, and consented field pilots.
- Phone-only reference accuracy is measured separately from workflows using control points or external survey.
- Long indoor sequences and return visits are first-class benchmarks.
- Visual quality and metric accuracy have separate scorecards.
...
| ------------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| RECBENCH-001 | P0       | The benchmark suite shall include trajectory ground truth, known dimensions, high-quality reference scans, semantic inventory, and change events where applicable.         | Automated test          |
| RECBENCH-002 | P0       | Every result shall record source code, model/checkpoint hash, environment, hardware, parameters, and input hashes.                                                         | Automated test          |
| RECBENCH-003 | P0       | Metrics shall include ATE/RPE, point-to-surface distance, completeness, normal error, scale drift, loop residual, change precision/recall, runtime, peak memory, and cost. | Controlled verification |
| RECBENCH-004 | P1       | Regressions beyond declared tolerances shall block release or require an explicit scoped waiver.                                                                           | Controlled verification |
| RECBENCH-005 | P1       | Benchmark datasets with people or homes shall document consent, access, retention, and publication restrictions.                                                           | Controlled verification |
| RECBENCH-006 | P1       | Results shall be stratified by scene type and not averaged into a misleading single number.                                                                                | Controlled verification |
    ...
    - A versioned `rec_bench_manifest` recording identity, timestamps, ownership, state, and provenance.
