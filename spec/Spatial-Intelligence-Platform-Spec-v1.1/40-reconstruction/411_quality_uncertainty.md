---
spec_id: REC-UNC
title: "Quality, Uncertainty, and Acceptance Classification"
version: 1.1.0
...
normative: true
---
# Quality, Uncertainty, and Acceptance Classification

    ## 1. Purpose

    Defines quantitative and qualitative measures for trajectory, depth, surface, coverage, registration, semantics, and visual fidelity.
    ...
    - Quality is multidimensional and cannot be reduced to one global confidence score.
- Uncertainty is stored at run, frame, pose, depth, surface-region, entity, assertion, and transform levels as appropriate.
- Acceptance states are failed, quarantined, preview, reviewed, accepted-reference, and verified-authoritative where permitted.
- Quality thresholds depend on intended use and capture profile.
...
| ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| RECUNC-001 | P0       | A scene revision shall carry trajectory, alignment, coverage, surface, source-contribution, semantic, and provenance quality summaries. | Controlled verification |
| RECUNC-002 | P0       | Quality metrics shall define units, calculation, calibration, thresholds, and known blind spots.                                        | Controlled verification |
| RECUNC-003 | P0       | A downstream workflow shall declare the minimum acceptance state it consumes.                                                           | Controlled verification |
| RECUNC-004 | P1       | The UI shall reveal region-level uncertainty and source coverage rather than only a project-level badge.                                | Controlled verification |
| RECUNC-005 | P1       | Human review shall record reviewer, evidence examined, decision, scope, and limitations.                                                | Controlled verification |
| RECUNC-006 | P1       | Verification shall never be inferred from elapsed time, customer payment, or absence of complaints.                                     | Controlled verification |
    ...
    - A versioned `rec_unc_manifest` recording identity, timestamps, ownership, state, and provenance.
