---
spec_id: REC-REPRO
title: "Pipeline Reproducibility and Run Manifests"
version: 1.1.0
...
normative: true
---
# Pipeline Reproducibility and Run Manifests

    ## 1. Purpose

    Defines complete lineage, deterministic modes, environment capture, artifact hashing, rerun behavior, and comparison reports.
    ...
    - A published derivative is invalid without a complete run manifest.
- Bitwise reproducibility is required for deterministic metadata transforms and targeted where feasible for numeric pipelines; tolerance-based reproducibility is documented for GPU operations.
- Containers, checkpoints, auxiliary models, schemas, and configuration are content-addressed.
- A rerun creates a new run and comparison; it never overwrites the original result.
...
| ------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| RECREPRO-001 | P0       | The run manifest shall include inputs, software commits, images, packages, container digest, hardware, driver/runtime, model manifests, parameters, seeds, locale/timezone, outputs, logs, metrics, and validation. | Controlled verification |
| RECREPRO-002 | P0       | The platform shall verify every referenced hash before executing a reproducibility claim.                                                                                                                           | Automated test          |
| RECREPRO-003 | P0       | A comparison report shall classify identical, within tolerance, materially different, and incomparable outputs.                                                                                                     | Controlled verification |
| RECREPRO-004 | P1       | Non-deterministic algorithms shall document expected variance and benchmark it.                                                                                                                                     | Controlled verification |
| RECREPRO-005 | P1       | Expired or revoked dependencies shall not prevent reading old manifests, but may prevent execution until isolated approval.                                                                                         | Automated test          |
| RECREPRO-006 | P1       | Release acceptance shall reproduce representative runs in a clean environment.                                                                                                                                      | Controlled verification |
    ...
    - A versioned `rec_repro_manifest` recording identity, timestamps, ownership, state, and provenance.
