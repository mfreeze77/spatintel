---
spec_id: REC-DEPTH
title: "Depth Observation Normalization and Fusion"
version: 1.1.0
...
normative: true
---
# Depth Observation Normalization and Fusion

    ## 1. Purpose

    Defines how LiDAR, ARKit, Polycam, LingBot, stereo, and imported depth are converted into source-aware metric observations.
    ...
    - Depth sources are never averaged without source-specific calibration, validity, and uncertainty.
- LiDAR normally dominates close-range metric surfaces when confidence and incidence are valid.
- Learned depth may fill gaps but its contribution remains identifiable and removable.
- Dynamic and reflective regions receive specialized filtering rather than ordinary static fusion.
...
| ------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| RECDEPTH-001 | P0       | Each depth map shall declare encoding, units/scale, camera model, intrinsics, pose, timestamp, valid mask, confidence model, and source manifest. | Controlled verification |
| RECDEPTH-002 | P0       | Unprojection shall be tested with synthetic cameras and known planes for every supported source convention.                                       | Controlled verification |
| RECDEPTH-003 | P0       | Weights shall account for range, confidence, viewing angle, motion, temporal agreement, calibration, and source class.                            | Controlled verification |
| RECDEPTH-004 | P1       | The pipeline shall retain source-contribution statistics per voxel/surface region.                                                                | Controlled verification |
| RECDEPTH-005 | P1       | Depth discontinuities shall not be blurred across semantic or edge boundaries without explicit algorithm behavior.                                | Controlled verification |
| RECDEPTH-006 | P1       | Fusion shall be repeatable from immutable normalized observations and a versioned policy.                                                         | Controlled verification |
    ...
    - A versioned `rec_depth_manifest` recording identity, timestamps, ownership, state, and provenance.
