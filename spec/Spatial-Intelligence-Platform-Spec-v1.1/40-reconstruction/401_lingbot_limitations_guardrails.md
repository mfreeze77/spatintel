---
spec_id: REC-LIM
title: "LingBot-Map Operating Envelope and Guardrails"
version: 1.1.0
...
normative: true
---
# LingBot-Map Operating Envelope and Guardrails

    ## 1. Purpose

    Translates upstream research limitations and observed failure modes into enforceable product controls.
    ...
    - SIP does not rely on LingBot-Map for explicit global loop closure.
- Sequences beyond the trained context range use keyframe and window policies plus independent cross-window optimization.
- Maximum distance, scene class, camera motion, and dynamic-content support are benchmarked rather than assumed.
- Low confidence or visually plausible output is not accepted without metric and trajectory checks.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| RECLIM-001 | P0       | The scheduler shall select fixed-cache, keyframe, or windowed execution from a versioned policy based on frame count, scene length, motion, and hardware.    | Controlled verification |
| RECLIM-002 | P0       | The pipeline shall detect cache/window discontinuities through shared-frame residuals and geometry overlap.                                                  | Controlled verification |
| RECLIM-003 | P0       | The validator shall compare learned trajectory to ARKit, IMU gravity, controls, and physical speed/acceleration bounds when available.                       | Controlled verification |
| RECLIM-004 | P1       | A run shall be marked outside operating envelope when its input exceeds validated distance, resolution, device motion, dynamic occupancy, or scene category. | Automated test          |
| RECLIM-005 | P1       | Product UI shall not expose unsupported scenes as verified simply because a point cloud rendered successfully.                                               | Controlled verification |
| RECLIM-006 | P1       | Benchmarks shall include long repeated corridors, loops, stairs, elevators, glass, mirrors, low texture, outdoor sky, and moving people.                     | Controlled verification |
    ...
    - A versioned `rec_lim_manifest` recording identity, timestamps, ownership, state, and provenance.
