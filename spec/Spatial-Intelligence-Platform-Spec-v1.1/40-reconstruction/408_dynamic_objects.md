---
spec_id: REC-DYN
title: "Dynamic People, Objects, and Scene Layers"
version: 1.1.0
...
normative: true
---
# Dynamic People, Objects, and Scene Layers

    ## 1. Purpose

    Defines detection, masking, tracking, static-background reconstruction, and optional temporal representation of moving content.
    ...
    - Structural geometry excludes transient people and movable objects unless intentionally modeled as entities.
- A memory scene may preserve a person or pet as temporal media while maintaining a separate static place model.
- Dynamic masks are confidence-aware and reviewable.
- Objects that move across sessions retain identity only when evidence supports the association.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| RECDYN-001 | P0       | The pipeline shall preserve original frames before masking.                                                              | Controlled verification |
| RECDYN-002 | P0       | Dynamic detections shall include model manifest, class, track, mask, confidence, and review state.                       | Controlled verification |
| RECDYN-003 | P0       | Static fusion shall exclude accepted dynamic pixels and report excluded coverage.                                        | Controlled verification |
| RECDYN-004 | P1       | The system shall distinguish unknown motion from sensor/pose error.                                                      | Controlled verification |
| RECDYN-005 | P1       | A person identity shall not be inferred or published solely from spatial track continuity.                               | Controlled verification |
| RECDYN-006 | P1       | Construction change detection shall not report ordinary temporary workers or equipment as structural changes by default. | Controlled verification |
    ...
    - A versioned `rec_dyn_manifest` recording identity, timestamps, ownership, state, and provenance.
