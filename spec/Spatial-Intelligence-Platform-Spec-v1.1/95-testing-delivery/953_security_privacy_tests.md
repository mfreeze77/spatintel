---
spec_id: TST-SEC
title: "Security, Privacy, Consent, and Abuse Testing"
version: 1.1.0
...
normative: true
---
# Security, Privacy, Consent, and Abuse Testing

    ## 1. Purpose

    Defines automated and adversarial tests for isolation, permissions, redaction, consent, injection, exports, sharing, deletion, and operations.
    ...
    - Cross-tenant and restricted-spatial-object leakage tests are mandatory.
- Prompt injection is tested across documents, transcripts, image text, entity names, and imported metadata.
- Consent is tested through every projection and derivative, not only the primary UI.
- Red-team findings create requirements and regression tests.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| TSTSEC-001 | P0       | Tests shall attempt unauthorized discovery through IDs, counts, search snippets, vectors, thumbnails, tiles, caches, logs, share links, signed URLs, exports, and agent tools. | Controlled verification |
| TSTSEC-002 | P0       | Consent restriction/revocation shall propagate to processing, viewer, search, agents, reports, notifications, exports, and future model training.                              | Controlled verification |
| TSTSEC-003 | P0       | File ingestion shall test malicious archives, media parsers, oversized dimensions, embedded scripts, and metadata injection.                                                   | Controlled verification |
| TSTSEC-004 | P1       | Agent tests shall cover destructive requests, role confusion, evidence fabrication, source-label removal, and speaking as an unconsented person.                               | Controlled verification |
| TSTSEC-005 | P1       | Deletion tests shall verify canonical, derivatives, indexes, caches, links, and backup expiry behavior.                                                                        | Controlled verification |
| TSTSEC-006 | P1       | Incident and support access controls shall be exercised through realistic scenarios.                                                                                           | Controlled verification |
    ...
    - A versioned `tst_sec_manifest` recording identity, timestamps, ownership, state, and provenance.
