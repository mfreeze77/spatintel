---
spec_id: TST-DATA
title: "Dataset and Benchmark Curation Plan"
version: 1.1.0
...
normative: true
---
# Dataset and Benchmark Curation Plan

    ## 1. Purpose

    Defines acquisition, consent, licensing, splits, ground truth, versioning, contamination checks, and publication for test datasets.
    ...
    - Benchmark data has its own manifest and governance independent of model/code repositories.
- Private customer or family data is not reused for general training/evaluation without explicit consent and agreement.
- Controlled physical scenes complement public datasets that may not represent construction or memories.
- Dataset revisions are immutable and results name exact version.
...
| ----------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| TSTDATA-001 | P0       | Each dataset shall document purpose, source, consent/rights, license, subjects/locations, sensitivities, allowed uses, retention, splits, ground truth, known bias, and hash manifest. | Automated test          |
| TSTDATA-002 | P0       | Train/tune/test leakage shall be assessed for any model adapted by the team.                                                                                                           | Controlled verification |
| TSTDATA-003 | P0       | Ground truth shall state equipment/method, calibration, uncertainty, alignment, and processing.                                                                                        | Controlled verification |
| TSTDATA-004 | P1       | Field pilots shall use data agreements that separate service delivery from model improvement.                                                                                          | Controlled verification |
| TSTDATA-005 | P1       | Public result publication shall respect location/security and personal privacy.                                                                                                        | Controlled verification |
| TSTDATA-006 | P1       | Dataset withdrawal shall identify affected models/results and trigger governance review.                                                                                               | Controlled verification |
    ...
    - A versioned `tst_data_manifest` recording identity, timestamps, ownership, state, and provenance.
