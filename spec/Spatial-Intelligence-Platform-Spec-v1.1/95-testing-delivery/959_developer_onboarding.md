---
spec_id: DEL-DEV
title: "Developer Onboarding and Local Environment"
version: 1.1.0
...
normative: true
---
# Developer Onboarding and Local Environment

    ## 1. Purpose

    Defines prerequisites, repository setup, fixtures, services, GPU optionality, tests, security, and first contribution path.
    ...
    - A developer can run core control plane, web, schemas, and fixture pipeline without a production GPU or customer data.
- GPU adapters are optional profiles with deterministic mock/reference workers.
- Secrets and private model checkpoints are never prerequisites for basic onboarding.
- The first exercise follows one capture fixture to one scene and evidence anchor.
...
| ---------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| DELDEV-001 | P0       | The repository shall provide version-managed toolchains, containerized dependencies, seed fixtures, one-command checks, and troubleshooting. | Controlled verification |
| DELDEV-002 | P0       | Local data shall be synthetic and clearly isolated from production.                                                                          | Controlled verification |
| DELDEV-003 | P0       | Developers shall complete secure-coding, data classification, truth/provenance, and model-license orientation.                               | Controlled verification |
| DELDEV-004 | P1       | The onboarding path shall validate schemas, run tests, ingest a fixture, execute mock metric/learned stages, and open the viewer.            | Automated test          |
| DELDEV-005 | P1       | GPU setup shall verify driver/runtime compatibility and use approved local checkpoints only.                                                 | Controlled verification |
| DELDEV-006 | P1       | Contributor documentation shall state code owners, review expectations, ADR process, and release gates.                                      | Controlled verification |
    ...
    - A versioned `del_dev_manifest` recording identity, timestamps, ownership, state, and provenance.
