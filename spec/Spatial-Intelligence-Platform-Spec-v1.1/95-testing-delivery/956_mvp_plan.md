---
spec_id: DEL-MVP
title: "MVP Definition and Build Plan"
version: 1.1.0
...
# MVP Definition and Build Plan
    ...
    Defines the smallest integrated product that proves shared spatial capture, reconstruction, evidence, viewer, Construction, and LiveForever value.
    ...
    - The MVP is one room/adjacent area per vertical, not an entire campus or lifetime.
- The same canonical package, pipeline, scene graph, viewer, identity, evidence, and export serve both demos.
- Manual semantic anchoring is acceptable before broad automated detection.
- Metric and evidence correctness outrank polished generation.
...
| ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| DELMVP-001 | P0       | MVP shall ingest first-party iPhone and Polycam raw capture fixtures.                                                                               | Controlled verification |
| DELMVP-002 | P0       | MVP shall run ARKit/LiDAR metric processing, a gated LingBot research adapter, alignment, TSDF/mesh, quality summary, and browser publication.      | Controlled verification |
| DELMVP-003 | P0       | MVP shall create stable entities, anchors, evidence, assertions, tasks, scene commits, diff, search, and open export.                               | Controlled verification |
| DELMVP-004 | P1       | Construction demo shall include a systems room, devices/door/equipment, drawing link, issue, measurement verification, and report.                  | Controlled verification |
| DELMVP-005 | P1       | LiveForever demo shall include a room, object, interview stories, old media, uncertainty/conflict, evidence view, consent, and preservation export. | Controlled verification |
| DELMVP-006 | P1       | MVP shall run local workstation or hybrid and document production blockers.                                                                         | Controlled verification |
    ...
    - A versioned `del_mvp_manifest` recording identity, timestamps, ownership, state, and provenance.
...
- A result summary carrying status, warnings, confidence, metrics, and links to detailed artifacts.
    ...
    1. Milestone 0: repository, schemas, CI, licenses, fixtures, local environment, threat model.
2. Milestone 1: canonical package, validators, object storage, project/capture APIs, Polycam adapter.
3. Milestone 2: iOS capture alpha with recovery, upload, quality events, and package finalization.
4. Milestone 3: metric lane, LingBot adapter, alignment, fusion, scene publication, quality review.
5. Milestone 4: scene graph, evidence, anchors, viewer, search, commits/diffs, exports.
6. Milestone 5: Construction and LiveForever workflows and dual demo acceptance.
7. Milestone 6: security/privacy hardening, benchmarks, backup/restore, cost controls, pilot readiness.
...
## Version 1.1 MVP extension

After the foundational capture/ingest/scene slice, the MVP adds explicit representation roles, one local proxy/collision baseline, quarantine and publication, native hybrid rendering, stable support maps, measurement re-resolution, one construction room, and one LiveForever memory room. Research and manual providers remain optional comparison lanes.
