---
spec_id: ARC-EVT
title: "Event-Driven Workflows and State Machines"
version: 1.1.0
...
normative: true
---
# Event-Driven Workflows and State Machines

    ## 1. Purpose

    Defines durable domain events, orchestration, idempotency, retries, cancellation, compensation, and publication ordering.
    ...
    - Events describe committed facts in past tense; commands express requested intent.
- Orchestration owns long-running workflow state and workers execute discrete activities.
- At-least-once delivery is assumed, so every consumer is idempotent.
- Cancellation is cooperative, audited, and cannot leave authoritative partial output.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| ARCEVT-001 | P0       | Every event shall include event ID, type, schema version, aggregate ID, tenant/project context, occurrence time, trace ID, producer, and payload hash. | Automated test          |
| ARCEVT-002 | P0       | Outbox or equivalent atomic publication shall prevent database commit/event divergence.                                                                | Automated test          |
| ARCEVT-003 | P1       | Retries shall distinguish transient, capacity, policy, invalid-input, and deterministic-algorithm failures.                                            | Controlled verification |
| ARCEVT-004 | P1       | Workflow state shall survive process restart and orchestrator failover.                                                                                | Controlled verification |
| ARCEVT-005 | P1       | Dead-letter items shall expose remediation and safe replay controls.                                                                                   | Controlled verification |
    ...
    - A versioned `arc_evt_manifest` recording identity, timestamps, ownership, state, and provenance.
