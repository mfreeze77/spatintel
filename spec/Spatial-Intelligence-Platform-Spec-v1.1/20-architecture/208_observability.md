---
spec_id: ARC-OBS
title: "Observability, Quality Telemetry, and Audit Correlation"
version: 1.1.0
...
normative: true
---
# Observability, Quality Telemetry, and Audit Correlation

    ## 1. Purpose

    Defines logs, metrics, traces, quality records, audit events, dashboards, alerts, and privacy-safe diagnostics.
    ...
    - Operational telemetry and immutable audit are separate systems with linked correlation IDs.
- Geometry quality metrics are first-class, not buried in worker logs.
- Raw sensitive content is excluded from normal telemetry.
- Every published scene revision links to the runs and validations that produced it.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| ARCOBS-001 | P0       | Logs shall be structured and include release, service, tenant-safe context, trace, operation, outcome, and stable error code.        | Controlled verification |
| ARCOBS-002 | P0       | Metrics shall identify model/checkpoint and capture profile without leaking personal or facility-identifying values.                 | Controlled verification |
| ARCOBS-003 | P1       | Distributed traces shall connect ingest, orchestration, worker, storage, validation, and publication.                                | Controlled verification |
| ARCOBS-004 | P1       | Quality dashboards shall show drift, coverage, residuals, confidence distribution, failed regions, and comparison to prior versions. | Controlled verification |
| ARCOBS-005 | P1       | Audit events shall be append-only and protected from ordinary support modification.                                                  | Automated test          |
    ...
    - A versioned `arc_obs_manifest` recording identity, timestamps, ownership, state, and provenance.
