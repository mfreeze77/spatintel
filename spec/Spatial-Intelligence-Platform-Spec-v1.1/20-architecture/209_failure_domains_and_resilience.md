---
spec_id: ARC-RES
title: "Failure Domains, Resilience, and Degraded Modes"
version: 1.1.0
...
normative: true
---
# Failure Domains, Resilience, and Degraded Modes

    ## 1. Purpose

    Defines how the platform contains failures in devices, networks, workers, models, stores, indexes, providers, and user workflows.
    ...
    - Canonical data stores are protected from compute-plane failure.
- Preview failure does not invalidate accepted raw capture; final-quality failure does not erase a successful preview.
- Derived services such as search and thumbnails can be rebuilt.
- A model regression can be disabled independently through manifest state and routing.
...
| ---------- | -------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------- |
| ARCRES-001 | P0       | Each component shall declare blast radius, retry safety, recovery point, recovery time, and degraded behavior.   | Controlled verification |
| ARCRES-002 | P0       | Worker resource exhaustion shall terminate the job without terminating shared control-plane services.            | Controlled verification |
| ARCRES-003 | P1       | Object-store write success shall be followed by hash verification before database publication.                   | Automated test          |
| ARCRES-004 | P1       | Index outage shall fall back to bounded metadata navigation rather than returning unauthorized or stale results. | Controlled verification |
| ARCRES-005 | P1       | Provider-region failure shall not trigger unapproved cross-region data movement.                                 | Controlled verification |
    ...
    - A versioned `arc_res_manifest` recording identity, timestamps, ownership, state, and provenance.
