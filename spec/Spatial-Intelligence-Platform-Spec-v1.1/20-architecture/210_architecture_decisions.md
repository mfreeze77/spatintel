...
# Architecture Decision Record Process
    ...
    Defines how consequential technical choices are proposed, tested, approved, superseded, and linked to implementation.
    ...
    - ADRs are required for canonical formats, coordinate conventions, state ownership, model selection, security boundaries, and incompatible dependencies.
- An ADR records context, options, decision, consequences, evidence, migration, and revisit trigger.
- Research choices may be provisional but must state production blockers.
- Superseding an ADR preserves the prior decision and affected-release history.
...
| ---------- | -------- | ----------------------------------------------------------------------------------------------------- | ----------------------- |
| ARCADR-001 | P0       | An ADR shall link to requirements, risks, benchmarks, and source evidence.                            | Controlled verification |
| ARCADR-002 | P0       | A vendor choice shall include an exit and export strategy.                                            | Controlled verification |
| ARCADR-003 | P1       | A model ADR shall include license, checkpoint, hardware, benchmark, operating envelope, and fallback. | Controlled verification |
| ARCADR-004 | P1       | Security and privacy reviewers shall approve decisions that expand data use or trust boundaries.      | Controlled verification |
| ARCADR-005 | P1       | CI shall validate ADR identifiers and links referenced by schemas or code ownership files.            | Automated test          |
    ...
    - A versioned `arc_adr_manifest` recording identity, timestamps, ownership, state, and provenance.
