---
spec_id: ARC-SVC
title: "Service Catalog and Ownership"
version: 1.1.0
...
# Service Catalog and Ownership
    ...
    Specifies each runtime service, its state ownership, API surface, events, dependencies, scaling unit, and failure impact.
    ...
    - Domain services own state; workers remain stateless except for resumable job scratch and checkpoints.
- Services are split by transaction and policy boundaries, not by arbitrary microservice fashion.
- The first deployment may co-locate compatible services while preserving logical boundaries.
- Every service publishes an operational contract and data-classification profile.
...
| ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| ARCSVC-001 | P0       | The catalog shall list service owner, repository path, data stores, inbound/outbound APIs, events, secrets, SLO, and recovery strategy. | Automated test          |
| ARCSVC-002 | P0       | A service shall not write tables owned by another service except through an approved shared-kernel exception.                           | Controlled verification |
| ARCSVC-003 | P1       | Worker callbacks shall be idempotent and authenticated as workload identities.                                                          | Automated test          |
| ARCSVC-004 | P1       | Service shutdown shall drain or safely requeue in-flight work.                                                                          | Controlled verification |
| ARCSVC-005 | P1       | Critical service dependencies shall have explicit timeout, circuit-breaker, and degradation behavior.                                   | Controlled verification |
    ...
    - A versioned `arc_svc_manifest` recording identity, timestamps, ownership, state, and provenance.
...
## Version 1.1 service-catalog additions

The catalog gains `representation-api`, `provider-registry`, `representation-publisher`, `splat-surface-worker`, `geometry-cleanup-worker`, `collision-navigation-worker`, and `representation-quality-worker`. Their authority, data ownership, trust boundaries, inputs, outputs, failure isolation, and SLO responsibilities are binding in [`211_hybrid_representation_services.md`](211_hybrid_representation_services.md).
