---
spec_id: ARC-DEP
title: "Deployment Profiles and Topology"
version: 1.1.0
...
normative: true
---
# Deployment Profiles and Topology

    ## 1. Purpose

    Defines local workstation, edge appliance, hybrid, single-tenant cloud, and multi-tenant cloud deployment patterns.
    ...
    - Canonical schemas and package formats are identical across deployment profiles.
- Local-only mode can complete capture, processing, review, and export without public internet after installation.
- Hybrid mode can pin raw assets to local or regional storage while sending only permitted derivatives to elastic compute.
- Cloud mode separates tenant workloads through identity, storage prefixes/buckets, database policy, queues, encryption, and network boundaries.
...
| ---------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| ARCDEP-001 | P0       | Each profile shall declare which features require cloud connectivity and what happens when unavailable.                            | Controlled verification |
| ARCDEP-002 | P0       | Deployment manifests shall pin service images, infrastructure versions, secrets references, network policies, and resource limits. | Controlled verification |
| ARCDEP-003 | P1       | Data residency policy shall be enforceable before asset upload or worker scheduling.                                               | Controlled verification |
| ARCDEP-004 | P1       | Local upgrades shall be reversible and preserve compatible export paths.                                                           | Controlled verification |
| ARCDEP-005 | P1       | Cloud autoscaling shall enforce tenant quotas and global budget limits.                                                            | Controlled verification |
    ...
    - A versioned `arc_dep_manifest` recording identity, timestamps, ownership, state, and provenance.
