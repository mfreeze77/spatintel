...
normative: true
---
# System Context and External Boundaries

    ## 1. Purpose

    Defines SIP's actors, external systems, trust boundaries, and data flows from mobile capture through reconstruction and experiences.
    ...
    - Mobile devices, browsers, desktop reviewers, external capture tools, BIM/document systems, identity providers, object storage, GPU providers, and notification services are explicit external boundaries.
- Raw media never transits a third-party AI API unless project policy and data classification permit it.
- Connectors import through adapters and cannot write authoritative scene state directly.
- Export is a first-class boundary with authorization, provenance, and redaction.
...
| ---------- | -------- | ----------------------------------------------------------------------------------------------------------------- | ----------------------- |
| ARCCTX-001 | P0       | The context model shall identify data controller/processor responsibility for each deployment profile.            | Controlled verification |
| ARCCTX-002 | P0       | Every inbound source shall pass schema, malware, decompression-bomb, and content-policy checks before processing. | Automated test          |
| ARCCTX-003 | P1       | Every outbound export shall apply permissions, consent, redaction, watermarking policy, and audit logging.        | Controlled verification |
| ARCCTX-004 | P1       | External service outages shall degrade bounded capabilities without corrupting canonical data.                    | Controlled verification |
| ARCCTX-005 | P1       | Trust-boundary changes shall require threat-model review.                                                         | Controlled verification |
    ...
    - A versioned `arc_ctx_manifest` recording identity, timestamps, ownership, state, and provenance.
