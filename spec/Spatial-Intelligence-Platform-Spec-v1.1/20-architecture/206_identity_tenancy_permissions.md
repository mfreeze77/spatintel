---
spec_id: ARC-IAM
title: "Identity, Tenancy, Authorization, and Sharing"
version: 1.1.0
...
normative: true
---
# Identity, Tenancy, Authorization, and Sharing

    ## 1. Purpose

    Defines users, service identities, tenants, projects, roles, scopes, consent, spatial restrictions, links, and delegated access.
    ...
    - Authorization is policy-based and context-aware rather than a single project role check.
- Viewer access and export/download access are separate permissions.
- Spatial redaction and restricted entities apply at query, API, viewer, and export layers.
- Public links reference redacted immutable presentations, not live unrestricted projects.
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| ARCIAM-001 | P0       | Every request shall be authorized using authenticated principal, tenant, project, action, resource classification, purpose, and relevant consent. | Controlled verification |
| ARCIAM-002 | P0       | Service identities shall use short-lived credentials and cannot impersonate users without explicit delegated context.                             | Controlled verification |
| ARCIAM-003 | P1       | Tenant isolation tests shall cover database, object storage, queues, caches, search, logs, and GPU scratch.                                       | Controlled verification |
| ARCIAM-004 | P1       | Share links shall have scope, expiry, revocation, watermark policy, and access logs.                                                              | Controlled verification |
| ARCIAM-005 | P1       | Permission changes shall invalidate cached access decisions within a bounded interval.                                                            | Automated test          |
    ...
    - A versioned `arc_iam_manifest` recording identity, timestamps, ownership, state, and provenance.
