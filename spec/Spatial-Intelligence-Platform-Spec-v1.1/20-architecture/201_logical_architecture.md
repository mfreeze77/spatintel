...
# Logical Architecture
    ...
    Partitions the platform into control plane, data plane, compute plane, experience plane, and governance plane.
    ...
    - The control plane owns identity, projects, state machines, manifests, jobs, policy, and publication.
- The data plane owns immutable objects, transactional metadata, spatial indexes, search indexes, and backups.
- The compute plane runs isolated CPU/GPU workers through versioned adapters.
- The experience plane includes capture, web viewer, desktop review, reports, and vertical workflows.
- The governance plane owns model approval, audit, consent, retention, risk, and release evidence.
...
| ---------- | -------- | -------------------------------------------------------------------------------------------------------------- | ----------------------- |
| ARCLOG-001 | P0       | A compute worker shall not receive direct database-owner credentials.                                          | Controlled verification |
| ARCLOG-002 | P0       | Publication of a scene revision shall be atomic with its manifest and audit event.                             | Automated test          |
| ARCLOG-003 | P1       | Derived indexes shall be rebuildable from canonical transactional records and object assets.                   | Controlled verification |
| ARCLOG-004 | P1       | Services shall expose health, readiness, version, and dependency status.                                       | Controlled verification |
| ARCLOG-005 | P1       | The architecture shall support replacing a model or storage vendor without changing stable domain identifiers. | Controlled verification |
    ...
    - A versioned `arc_log_manifest` recording identity, timestamps, ownership, state, and provenance.
...
The logical architecture now includes a provider registry/policy gate, splat-surface workers, geometry-cleanup workers, collision/navigation compilers, quarantine storage, representation-quality gates, an atomic representation publisher, and a hybrid scene runtime. These components are defined in [`211_hybrid_representation_services.md`](211_hybrid_representation_services.md). Provider workers emit candidates only; they cannot alter evidence, authority, consent, or accepted scene commits.
